...
- 版本基线：1.50.3（线上已发布 v1.50.3；工作区待提交：图片体验三项优化 + 截图插入链路统一 + 阅读进度恢复修复 + 右键“插入图片”入口 + Agent 交接体系更名）。
- 仓库规则：见根目录 `AGENTS.md`；每轮代码交付三份 ZIP（源码 / 安装 / Agent 交接）共用同一六位后缀；验证入口 `npm run verify`。

## 当前状态（工作区待提交：图片体验三项优化 + 截图插入链路统一 + 阅读进度恢复修复）

- 图片体验三项优化：(1) “本地副本”文案统一改为“本地图片”（预览来源栏 label、设置项、通知与注释）；(2) 图片预览“更新替换本地图片”后旧本地文件进入 60 秒延迟回收（复用文件块回收机制：撤销或重新引用自动取消、到期全库引用检查后移入系统回收站），同时补齐图片块删除、节点删除场景的旧图片回收，`deleteFileAssetIfSafe` 全库引用检查扩展到图片块 localSource；(3) 图片块右键新增“在文件资源管理器中显示”（有 localSource 时显示），main.ts 新增 `revealFileInSystemExplorer`：动态 require electron `shell.showItemInFolder` 并用 `FileSystemAdapter.getFullPath` 换算绝对路径，移动端提示不支持。回调 `onRevealFileInSystemExplorer` 经 editor-types/view.ts 注入。
...
- `npm run verify` 本机完整通过：`test:unit` 427/427；`test:regression` 全部通过；`test:docs` 覆盖 63 个源码模块、1280 个具名声明；`test:repo` 通过；production esbuild 通过，`main.js` 已重建。
- 详细数据见根目录 `TEST_RESULTS.md`。

## 待验证事项（需真实 Obsidian 桌面端手工冒烟）

- 图片体验三项：预览来源栏与设置项显示“本地图片”；替换本地图片 60 秒后旧文件进入系统回收站（替换后立即撤销则取消回收）；图片块右键“在文件资源管理器中显示”打开目录并选中文件。
