...
class MindMapStudioPlugin extends obsidian_1.Plugin {
    ...
    async onload() {
        ...
        const pluginDir = this.manifest.dir ?? (0, obsidian_1.normalizePath)(`${this.app.vault.configDir}/plugins/${this.manifest.id}`);
        ...
        this.addCommand({
            ...
            checkCallback: (checking) => {
                ...
                if (!checking && available && file)
                    void this.openAsMindMap(file, this.app.workspace.activeLeaf ?? undefined);
                return available;
            }
        });
    ...
    findArticleNodeDepth(root, nodeId, baseDepth = 0) {
        return (0, modes_1.buildArticleNodeInfo)(root, baseDepth).find((entry) => entry.node.id === nodeId)?.depth ?? null;
    }
    /**
    ...
    async computeArticleBaseDepth(file, document, visited = new Set()) {
        if (visited.has(file.path) || !document.navigation?.parentPath)
            return 0;
        ...
        if (!parentNodeId) {
            ...
            parentNodeId = (0, model_1.flattenNodes)(parentDocument.root).find((node) => {
                if (!node.submap?.path)
                    return false;
    ...
    async readImageSource(source, sourceFile) {
        ...
        const file = direct instanceof obsidian_1.TFile ? direct : this.app.metadataCache.getFirstLinkpathDest(target, sourceFile?.path ?? "");
        if (!(file instanceof obsidian_1.TFile))
            return null;
    ...
    async uploadImageToHosts(blob, suggestedName, hostIds) {
        ...
        const hosts = requested
            ...
            .filter((host) => Boolean(host?.enabled && host.endpoint.trim()));
        if (!hosts.length)
    ...
    async runAutoUploadTask(mindMapPath, nodeId, blockId, localPath, suggestedName, hostIds) {
        try {
            ...
            const block = node?.content?.find((item) => item.type === "image" && item.id === blockId);
    ...
    async flushOpenView(path) {
        for (const leaf of this.app.workspace.getLeavesOfType(view_1.VIEW_TYPE_MINDMAP_STUDIO)) {
            if (leaf.view instanceof view_1.MindMapStudioView && leaf.view.file?.path === path)
    ...
    async refreshOpenMindMap(file, document) {
        ...
        for (const leaf of this.app.workspace.getLeavesOfType(view_1.VIEW_TYPE_MINDMAP_STUDIO)) {
            if (leaf.view instanceof view_1.MindMapStudioView && leaf.view.file?.path === file.path)
    ...
    mimeFromFilename(filename) {
        ...
        return { png: "image/png", jpg: "image/jpeg", jpeg: "image/jpeg", gif: "image/gif", webp: "image/webp", svg: "image/svg+xml", bmp: "image/bmp", avif: "image/avif" }[extension ?? ""] ?? "application/octet-stream";
    }
    /**
    ...
    async migrateLegacyFile(file, openAfter = true) {
        ...
        try {
            ...
            if (openAfter)
                await this.openAsMindMap(target, this.app.workspace.activeLeaf ?? undefined);
            return target;
        }
        catch (error) {
    ...
    async mergeFromSubmap(submapFile) {
        ...
        const searchParent = (node) => {
            ...
            if (node.submap?.path) {
                ...
                if (resolved?.path === submapFile.path) {
                    targetNode = node;
                    return;
                }
            }
