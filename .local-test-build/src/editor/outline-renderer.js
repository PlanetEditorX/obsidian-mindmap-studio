...
function renderOutlineMode(container, options) {
    ...
    const visit = (node, depth) => {
        ...
        const label = (0, model_1.nodePlainText)(node) || (node.submap?.title ?? "图片节点");
        if (node.submap) {
        ...
        if (node.articleNumberingMode === "manual") {
            row.createSpan({ cls: "mms-outline-badge", text: `文章层级 ${node.articleNumberingLevel ?? 1} · ${(0, modes_1.articleNumberLabel)(node.articleNumberingLevel ?? 1, 1)}` });
        }
...
function renderOutlineContent(container, node, depth, options) {
    ...
    for (const block of images) {
        ...
        if (resolved) {
            ...
            image.addEventListener("click", () => new editor_modals_1.ImagePreviewModal(options.app, resolved, block.alt ?? "图片", (0, model_1.imageSourceCandidates)(block, true), options.resolveImage).open());
        }
        else {
