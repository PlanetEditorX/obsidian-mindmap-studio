...
function parseClipboardNodes(text) {
    try {
        ...
        if (!inputs.length)
            return null;
        return inputs.map((input) => (0, model_1.normalizeDocument)({ title: input.text ?? "粘贴节点", root: input }, input.text ?? "粘贴节点").root);
    }
    catch {
...
function parseClipboardHtml(html) {
    ...
    const parseItem = (item) => {
        ...
        const node = (0, model_1.createNode)(clone.textContent?.trim() || "节点");
