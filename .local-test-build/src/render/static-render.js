...
function renderStaticMindMap(container, document, options) {
    ...
    const svg = (0, layout_1.documentToSvg)(document.root, document.layout, document.title, (0, model_1.mergeAppearance)(options?.defaultAppearance, document.appearance));
    ...
    if (options?.app && options.file) {
        image.addEventListener("dblclick", () => {
            void options.app?.workspace.getLeaf(false).openFile(options.file);
        });
