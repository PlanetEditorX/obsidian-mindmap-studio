...
function fieldValues(node) {
    ...
    if (node.submap?.path)
        values.push({ kind: "子导图", value: `${node.submap.title ?? ""} ${node.submap.path}` });
    if (node.code)
        values.push({ kind: "代码", value: `${node.code.language ?? ""}\n${node.code.code}` });
    if (node.table)
...
function mergeHierarchy(prefix, suffix) {
    ...
    for (const item of right) {
        if (normalized(merged.at(-1) ?? "") === normalized(item))
            continue;
        merged.push(item);
    }
    return merged;
...
function resolveHierarchicalEntries(files) {
    ...
    const resolveLineage = (filePath, visiting = new Set()) => {
        ...
        if (visiting.has(path)) {
            const fallback = [navigation.parentTitle, navigation.parentNodeText].filter((item) => Boolean(item?.trim()));
        ...
        const parentLocalPath = sourceEntry?.breadcrumb?.length
            ...
            : [parentFile.title, navigation.parentNodeText].filter((item) => Boolean(item?.trim()));
...
function searchEntries(entries, query, limit = 100, useRegex = false) {
    ...
    for (const entry of entries) {
        ...
        if (breadcrumb.includes(phrase))
            score += 70;
        if (normalized(entry.tags?.join(" ") ?? "").includes(phrase))
            score += 100;
        if (normalized(entry.note ?? "").includes(phrase))
            score += 60;
...
function collectIndexedFamilyPaths(files, rootPath) {
    ...
    while (queue.length) {
        const path = (0, obsidian_1.normalizePath)(queue.shift() ?? "");
    ...
    async refreshFamily(rootPath, currentDocument) {
        ...
        while (queue.length) {
            ...
            for (const [candidatePath, indexed] of Object.entries(this.data.files)) {
                ...
                if (resolvedParent?.path === path && !family.has(candidatePath))
                    queue.push(candidatePath);
            }
        }
