...
class MindMapStudioView extends obsidian_1.TextFileView {
    ...
    getViewData() {
        ...
        return (0, model_1.serializeDocument)(document ?? this.plugin.createConfiguredDocument("思维导图"));
    }
    /**
        ...
        if (!this.editor || clear) {
            ...
            this.editor = new editor_1.MindMapEditor(this.app, this.contentEl, this.document, {
                ...
                onRenderCode: async (block, container) => {
                    ...
                    await obsidian_1.MarkdownRenderer.render(this.app, markdown, container, this.file?.path ?? "", this);
                }
    ...
    async refreshArticleContext() {
        ...
        try {
            ...
            if (token !== this.articleContextToken || this.file?.path !== file.path)
                return;
