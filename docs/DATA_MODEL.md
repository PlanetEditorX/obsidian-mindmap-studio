...
文章样式还可保存 `leafNumberingEnabled` 与 `leafNumberingThreshold`，用于覆盖插件全局的末端正文转序号开关和阈值。达到阈值的末端兄弟节点使用下一层文章编号；阈值未达到或已处于第 8 级时继续使用末端正文标识。自动结构可以继续深入到第 9 级及以后，但这些层级只保留结构深度，不再循环复用第 7、8 级的 `A.` / `（A）` 标签。
