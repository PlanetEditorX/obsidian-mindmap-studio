...
- `articleNumberingMode` 缺失时表示自动；`none` 表示关闭编号；`manual` 表示使用手动文章层级。普通节点的 `none` 只跳过当前节点；根节点的 `none` 是当前物理导图的整页编号开关，关闭该文件内全部章节与末端自动序号。
- `articleNumberingLevel` 仅在手动模式下生效，规范化时限制为 `1–8`。
- 手动模式只覆盖当前标题或中心节点子树的最高文章层级，并让后代从该层级继续递增；它不改变导图中的真实 `children` 结构，也不强制孤立末端节点成为标题。
- 中心节点的手动层级直接作为整张导图一级内容的最高可见层级，中心标题本身仍不添加编号；中心节点关闭编号时，结构深度继续计算，但该物理导图的文章正文、文章目录、导出及子文章页标题不生成编号。通读模式拥有独立全书编号，会忽略中心节点的关闭状态，同时继续尊重普通节点的关闭和手动层级。
