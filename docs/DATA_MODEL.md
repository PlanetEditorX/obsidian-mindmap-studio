...
交互层不改变上述模型语义：导图模式使用鼠标拖动；文章编辑模式使用持久的键盘移动状态。文章上下键将相邻块作为前后目标，左键以直接父节点末尾为目标，右键以第一个直接子节点末尾为目标。每次移动仍只调用 `moveNodeContentBlock()`，因此块 ID、兼容字段重建、撤销保存和空来源节点清理规则保持一致。
