...
导入文本会在 Markdown 行内样式解析后移除常见的“一、”“（一）”“1.”等章节序号，因此序号位于加粗等标记内部时也不会残留；文章模式可按当前层级重新生成编号。整行仅用于返回目录或顶部的 Obsidian 块锚点不会生成节点，其他指向 `#^块ID` 的链接只保留可读标签，不保留原笔记锚点。桌面文件导入会把可读取的本地图片复制进当前导图资源目录，并通过 `replaceNodeContentBlocks()` 将新路径写回权威图片块的 `source/localSource`，同时重建 `node.image` 兼容镜像；仅修改 `nodeContentBlocks()` 返回的规范化副本不会持久化。 XMind 图片使用相同的权威图片块：解析阶段只写入临时资源令牌，保存到仓库后一次性改写为本地路径；共享归档资源只落盘一次。XMind 公式作为标题后的独立文字内容块保存，源码统一为单层 `$$...$$`，不以公式预览图片替代可编辑 LaTeX。无法读取时保留原始引用，不修改数据版本。图片进入最终导入节点树并获得稳定节点 ID 后，现有图床自动上传排程会按全局设置继续处理这些本地副本。渲染新复制资源时先按完整仓库路径直接读取，再回退到 Obsidian 链接缓存，避免缓存尚未索引时短暂加载失败。
