...
表格与代码可作为有稳定 ID 的内容块，与文字、图片一起排序；旧文件中的节点级 `table`、`code` 字段在读取时会自动迁移到内容块列表。表格行会按表头列数补齐或截断。代码块保留语言名称，并可保存默认折叠、行号和 Obsidian、GitHub、Monokai、Dracula 样式；渲染时交给 Obsidian Markdown 渲染器进行语法高亮。
