...
- `articleNumberingMode` 缺失时表示自动；`none` 表示关闭编号；`manual` 表示使用手动文章层级。普通节点的 `none` 只跳过当前节点；根节点的 `none` 是当前导图的整页编号开关，关闭该文件内全部章节与末端自动序号。文章族上下文会把顶层或祖先导图的关闭状态写入 `ReadingSection.numberingDisabled`，使挂载子导图保持同一关闭语义。
- `articleNumberingLevel` 仅在手动模式下生效，规范化时限制为 `1–8`。
- 手动模式只覆盖当前标题或中心节点子树的最高文章层级，并让后代从该层级继续递增；它不改变导图中的真实 `children` 结构，也不强制孤立末端节点成为标题。
- 中心节点的手动层级直接作为整张导图一级内容的最高可见层级，中心标题本身仍不添加编号；中心节点关闭编号时，结构深度继续计算，但该导图的正文、目录、通读、导出及子文章页标题均不生成编号。若它是文章族祖先，该关闭状态继续传给全部挂载后代。
...
题目公式不增加新数据类型：题干、选项、答案和解答仍保存为普通文字内容块，行内公式写成 `$...$`，独立公式写成 `$$...$$`。独立布局只在整个文字块仅包含双美元公式时生效；嵌入说明文字的历史双美元内容按行内恢复。含中文标签的公式只在 MathJax 渲染副本中添加 `\text{...}`，持久化源码保持原样。因此公式可与前后文字混排，也能继续经过搜索、导出、题库练习和文章/通读渲染链路；右键公式编辑器只是向当前光标或选区写入上述分隔符。
...
手工或脚本批量生成题库文件（`.mindmap` 原生 JSON 或 Markdown 内嵌 `mindmap-json` 围栏）时，只需写最小字段集，缺失字段会在加载时自动补全；字段写法约束、判题规则与导入核对清单见 [QUESTION_BANK.zh-CN.md](QUESTION_BANK.zh-CN.md)。
...
跨节点移动后，即使来源节点已经没有任何有效内容块，也仍保留该节点及其树位置；内容块移动只改变块的归属与顺序，不再隐式删除来源节点。空白文字占位块仍可在移动时被清理，但这不会改变节点本身是否存在。

删除内容同样不再触发节点删除：删除最后一个有效文字、图片、表格或代码块后，根节点和普通节点都会保留为空节点。节点树的移除只由显式“删除节点”等结构操作完成，因此清空内容和删除结构现在具有清晰、独立的语义。

交互层遵循同一模型语义：导图编辑模式的块手柄负责开始拖动，目标块上半区和下半区分别对应前后插入，节点空白区域对应末尾追加。文章编辑模式不显示块或整节点的悬浮拖动手柄；“作为块移动”进入临时选位模式，目标块上半区/下半区分别对应 `before`/`after`，节点空白区域对应 `append`，因此同时支持跨节点精确插入和同节点排序，且始终只调用 `moveNodeContentBlock()`。“作为节点移动”固定插入所选目标之后。“降为上一个节点的子节点”移动到同级前一节点之下；“升为上一个节点的兄弟节点”移动到父节点之后。上述操作均保留块 ID、兼容字段重建和撤销保存；来源节点即使最终为空也继续存在，等待后续输入或显式删除。
...
导入文本会在 Markdown 行内样式解析后移除常见的“一、”“（一）”“1.”等章节序号，因此序号位于加粗等标记内部时也不会残留；文章模式可按当前层级重新生成编号。整行仅用于返回目录或顶部的 Obsidian 块锚点不会生成节点，其他指向 `#^块ID` 的链接只保留可读标签，不保留原笔记锚点。桌面文件导入会把可读取的本地图片复制进当前导图资源目录，并通过 `replaceNodeContentBlocks()` 将新路径写回权威图片块的 `source/localSource`，同时重建 `node.image` 兼容镜像；仅修改 `nodeContentBlocks()` 返回的规范化副本不会持久化。 XMind 图片使用相同的权威图片块：解析阶段只写入临时资源令牌，保存到仓库后一次性改写为本地路径；共享归档资源只落盘一次。跨画布同名挂载不会只复制子节点，而是把被链接根主题除重复标题外的图片、公式和备注并入挂载节点，使任意嵌套深度的根内容仍属于同一权威节点。XMind 公式作为标题后的独立文字内容块保存，源码统一为单层 `$$...$$`，不以公式预览图片替代可编辑 LaTeX。无法读取时保留原始引用，不修改数据版本。图片进入最终导入节点树并获得稳定节点 ID 后，现有图床自动上传排程会按全局设置继续处理这些本地副本。渲染新复制资源时先按完整仓库路径直接读取，再回退到 Obsidian 链接缓存，避免缓存尚未索引时短暂加载失败。
