...
交互层不改变上述模型语义：导图和文章编辑模式的内容块继续共用鼠标拖动链路。块手柄负责开始拖动，目标块上半区和下半区分别对应前后插入，节点空白区域对应末尾追加；完全没有内容块的节点也以整个节点或文章章节作为可投放区域。文章章节另有“拖动整个节点”手柄，章节上沿、中部和下沿分别表示同级前置、成为子节点和同级后置，不会把标题误移为目标节点的内容块。内容块移动仍只调用 `moveNodeContentBlock()`，因此块 ID、兼容字段重建、撤销保存和空来源节点清理规则保持一致；只剩空文字占位块的来源叶节点也按真正空节点清理。

Markdown 导入会在解析标题、列表和普通文本时立即创建带稳定 `id` 的文字内容块，而不是只写入旧版 `text/richText` 镜像。文章模式因此可在首次渲染、编辑和失焦保存之间定位同一文字块。对于尚未经过规范化、只有旧版文字字段的文档，保存端会把渲染时的兼容文字块视为原块并原位更新，避免把修改内容追加成第二段。
