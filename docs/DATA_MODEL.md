...
交互层不改变上述模型语义：导图和文章编辑模式共用鼠标拖动链路。块手柄负责开始拖动，目标块上半区和下半区分别对应前后插入，节点空白区域对应末尾追加；完全没有内容块的节点也以整个节点或文章章节作为可投放区域。每次移动仍只调用 `moveNodeContentBlock()`，因此块 ID、兼容字段重建、撤销保存和空来源节点清理规则保持一致。
