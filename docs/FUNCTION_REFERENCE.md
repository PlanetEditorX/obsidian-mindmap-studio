...
### 函数 `reconcileReadingNodeInfo`

源码：`src/article/modes.ts:261`

使用全书目录中已经解析完成的跨文件编号结果修正通读正文节点。 通读目录会把父导图的本地子节点和子导图根节点下的内容放入同一结构层级计算； 若正文在每个物理文件内再次独立判断标题，会出现目录已有“一、”等编号、正文却仍 被当作普通段落的分叉。存在目录项时，以目录的层级、编号和标题分类作为权威结果； 未进入目录的末端正文仍保留本地末端编号与段落规则。
...
### 函数 `buildReadingArticleNodeInfo`

源码：`src/article/modes.ts:448`

构建通读模式使用的章节信息。通读是一套独立的全书编号视图，因此只忽略 当前物理导图中心节点上的整页“关闭编号”；普通节点的关闭与手动层级仍照常生效。 这样文章页可保持无编号，而同一导图进入通读后仍按全书结构显示章、节与末端序号。

```ts
export function buildReadingArticleNodeInfo( root: MindMapNode, baseDepth = 0, leafNumbering: ArticleLeafNumberingOptions =
```

### 函数 `normalizeVisibleModes`

源码：`src/article/modes.ts:463`
...
源码：`src/editor/article-renderer.ts:262`
...
源码：`src/editor/article-renderer.ts:323`
...
源码：`src/editor/article-renderer.ts:336`
...
源码：`src/editor/article-renderer.ts:341`
...
源码：`src/editor/article-renderer.ts:359`
...
源码：`src/editor/article-renderer.ts:383`
...
源码：`src/editor/article-renderer.ts:406`
...
源码：`src/editor/article-renderer.ts:476`
...
源码：`src/editor/article-renderer.ts:545`
...
源码：`src/editor/article-renderer.ts:587`
...
### 方法 `MindMapEditor.mutatePageAppearance`
