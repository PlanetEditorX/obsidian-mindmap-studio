...
Searchable current-map question bank with status and tag filters.
...
源码：`src/editor/question-bank-modal.ts:25`

Creates the bank over one document and delegates navigation and learning-state updates.

```ts
constructor( app: App, private readonly document: MindMapDocument, private readonly onOpenNode: (nodeId: string) => void, private readonly onSetStatus: (nodeId: string, status: MindMapQuestionStatus) => void )
```

### 方法 `QuestionBankModal.onOpen`

源码：`src/editor/question-bank-modal.ts:35`
...
源码：`src/editor/question-bank-modal.ts:41`
...
源码：`src/editor/question-bank-modal.ts:63`
