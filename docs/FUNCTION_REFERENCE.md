...
```ts
export async function requestAiCompletion( profile: AiProfileConfig, payload: AiMarkdownPayload, question: string, onStreamUpdate?: (update: AiStreamUpdate) => void, signal?: AbortSignal ): Promise<AiCompletionResult>
```
...
```ts
export async function requestAiEditProposal( profile: AiProfileConfig, payload: AiMarkdownPayload, instruction: string, onStreamUpdate?: (update: AiStreamUpdate) => void, signal?: AbortSignal ): Promise<AiCompletionResult>
```
...
```ts
export async function requestAiImageRecognition( profile: AiProfileConfig, image: Blob | string, prompt: string, signal?: AbortSignal ): Promise<AiCompletionResult>
```
...
### 方法 `AiAskModal.beginRequestSignal`

源码：`src/ai/modal.ts:71`
...
### 函数 `isAiRequestCancelled`

源码：`src/ai/protocol.ts:281`

判断错误是否由请求被主动取消（`AbortSignal`）引起，而不是真实的网络或服务端失败。 Electron 渲染进程抛出的取消错误可能是 `DOMException: AbortError`， 也可能是其他运行时包装后的普通 `Error`，因此同时匹配名称与常见消息文本。

```ts
export function isAiRequestCancelled(error: unknown): boolean
```

### 函数 `throwIfSignalAborted`

源码：`src/ai/protocol.ts:294`
...
### 函数 `createAiAbortError`

源码：`src/ai/protocol.ts:300`
...
### 接口 `AiSseEventAccumulator`

源码：`src/ai/protocol.ts:309`
...
### 函数 `createAiSseEventAccumulator`

源码：`src/ai/protocol.ts:315`

创建供完整文本解析与流式读取共用的 SSE 事件累计器。

```ts
function createAiSseEventAccumulator( defaultModel: string, onStreamUpdate?: (delta: AiStreamDelta) => void ): AiSseEventAccumulator
```

### 接口 `AiStreamReaderLike`

源码：`src/ai/protocol.ts:345`
...
### 函数 `consumeAiStreamReader`

源码：`src/ai/protocol.ts:355`

从浏览器 Fetch 读取器逐块消费 OpenAI 兼容 SSE 流。 读取器在请求被 `AbortSignal` 取消时会拒绝读取，错误原样向上传播； 流结束后仍会处理最后一个未以空行结尾的事件，行为与常见 SSE 客户端一致。

```ts
export async function consumeAiStreamReader( reader: AiStreamReaderLike, options:
```

## `src/article/article-context-cache.ts`

文章族上下文与已解析文档的同步内存缓存、依赖版本校验和插件私有 JSON 持久化。

### 接口 `ArticleContextCacheAdapter`

源码：`src/article/article-context-cache.ts:17`
...
### 接口 `MindMapFileRevision`

源码：`src/article/article-context-cache.ts:25`
...
### 接口 `ArticleContextData`

源码：`src/article/article-context-cache.ts:32`
...
### 接口 `ArticleContextCacheSnapshot`

源码：`src/article/article-context-cache.ts:41`
...
### 接口 `PersistedArticleContextCache`

源码：`src/article/article-context-cache.ts:52`
...
### 接口 `MindMapDocumentCacheEntry`

源码：`src/article/article-context-cache.ts:58`
...
### 函数 `normalizeArticleContextCachePath`

源码：`src/article/article-context-cache.ts:64`
...
源码：`src/article/article-context-cache.ts:69`
...
源码：`src/article/article-context-cache.ts:75`
...
源码：`src/article/article-context-cache.ts:80`
...
源码：`src/article/article-context-cache.ts:85`
...
### 函数 `normalizeTocEntry`

源码：`src/article/article-context-cache.ts:90`
...
源码：`src/article/article-context-cache.ts:106`
...
### 函数 `normalizeReadingSection`

源码：`src/article/article-context-cache.ts:122`
...
### 函数 `normalizeArticleContext`

源码：`src/article/article-context-cache.ts:137`
...
### 函数 `normalizeRevision`

源码：`src/article/article-context-cache.ts:156`
...
### 函数 `normalizeArticleContextCacheSnapshot`

源码：`src/article/article-context-cache.ts:173`
...
### 函数 `sameMindMapFileRevision`

源码：`src/article/article-context-cache.ts:199`
...
### 类 `ArticleContextCacheStore`

源码：`src/article/article-context-cache.ts:208`
...
### 构造函数 `ArticleContextCacheStore.constructor`

源码：`src/article/article-context-cache.ts:216`

创建一个以插件私有 JSON 文件为后端的有界缓存。

```ts
constructor( private readonly adapter: ArticleContextCacheAdapter, private readonly cacheDirectory: string, private readonly cacheFile: string )
```

### 方法 `ArticleContextCacheStore.initialize`

源码：`src/article/article-context-cache.ts:223`
...
### 方法 `ArticleContextCacheStore.get`

源码：`src/article/article-context-cache.ts:258`
...
### 方法 `ArticleContextCacheStore.put`

源码：`src/article/article-context-cache.ts:280`
...
### 方法 `ArticleContextCacheStore.clear`

源码：`src/article/article-context-cache.ts:309`
...
### 方法 `ArticleContextCacheStore.invalidateDependency`

源码：`src/article/article-context-cache.ts:316`
...
### 方法 `ArticleContextCacheStore.flush`

源码：`src/article/article-context-cache.ts:328`
...
### 方法 `ArticleContextCacheStore.markDirty`

源码：`src/article/article-context-cache.ts:338`
...
### 方法 `ArticleContextCacheStore.schedulePersist`

源码：`src/article/article-context-cache.ts:344`
...
### 方法 `ArticleContextCacheStore.startPersistRunner`

源码：`src/article/article-context-cache.ts:354`
...
### 方法 `ArticleContextCacheStore.runPersistLoop`

源码：`src/article/article-context-cache.ts:367`
...
### 方法 `ArticleContextCacheStore.prune`

源码：`src/article/article-context-cache.ts:387`
...
### 方法 `ArticleContextCacheStore.snapshotCharacters`

源码：`src/article/article-context-cache.ts:404`
...
### 类 `MindMapDocumentCache`

源码：`src/article/article-context-cache.ts:412`
...
### 方法 `MindMapDocumentCache.get`

源码：`src/article/article-context-cache.ts:416`
...
### 方法 `MindMapDocumentCache.put`

源码：`src/article/article-context-cache.ts:430`
...
### 方法 `MindMapDocumentCache.remove`

源码：`src/article/article-context-cache.ts:445`
...
### 函数 `resolveParentReturnIntent`

源码：`src/article/display-mode.ts:17`
...
源码：`src/article/display-mode.ts:22`
...
源码：`src/article/display-mode.ts:27`
...
源码：`src/article/display-mode.ts:38`
...
源码：`src/article/display-mode.ts:48`
...
源码：`src/article/display-mode.ts:58`
...
### 接口 `ArticleContextProgress`

源码：`src/article/modes.ts:44`

文章族上下文构建过程中向视图层上报的阶段性进度。 百分比用于右下角加载提示，计数用于在动态发现子导图时保持进度稳定。

```ts
export interface ArticleContextProgress
```

### 函数 `resolveArticleContextProgressPercent`

源码：`src/article/modes.ts:56`

将文章族上下文的已完成工作量映射为稳定百分比，避免初始加载期间 反复回退，同时为右下角加载提示提供可测试的统一计算边界。
...
### 函数 `refreshArticleTocEntriesFromReadingSections`

源码：`src/article/modes.ts:293`

Recomputes article TOC titles, breadcrumbs, and numbering from the already loaded continuous-reading family. This path performs no vault reads and is intended for text-only edits that cannot change family topology or article numbering settings. The persisted TOC intentionally points a mounted heading at its child file and clears `nodeId`. Because that loses the source mount-node identity, this helper preserves the existing entry identity by stable traversal position. A mismatch in entry count/depth means the mutation was not content-only and the caller must fall back to a complete article-context rebuild.

```ts
export function refreshArticleTocEntriesFromReadingSections( sections: readonly ReadingSection[], previousEntries: readonly ArticleTocEntry[] ): ArticleTocEntry[] | null
```

### 类型 `Item`

源码：`src/article/modes.ts:307`
...
源码：`src/article/render-window.ts:12`
...
源码：`src/article/render-window.ts:29`
...
源码：`src/article/render-window.ts:61`
...
### 接口 `MindMapFileContentBlock`

源码：`src/core/model.ts:280`

MindMapFileContentBlock 的结构化数据约定：节点附件文件块。 `source` 是仓库内附件文件的权威相对路径（与图片落盘同目录规则）； `name` 保留原始文件名用于展示与搜索；`size` 为可选字节大小。 删除块或节点时文件由插件层延迟 60 秒安全回收（进系统回收站）。
...
源码：`src/core/model.ts:986`

为图片内容块构建有序、去重的加载候选列表。默认优先使用本地图片（只要存在且未手动指定 默认来源）；用户通过图片级来源优先级（`sourcePriority`）手动固定的来源永远排最前；本地 图片不存在时才按图床优先级与镜像顺序加载远程镜像，从而支持失效图床自动切换。
...
### 函数 `normalizeImageSourcePriority`
...
```ts
export function removeImageSourceCandidate( block: MindMapImageContentBlock, source: string ): Pick<MindMapImageContentBlock, "source" | "localSource" | "remoteSources" | "sourcePriority"> | null
```

### 函数 `createManualImageRemoteSource`

源码：`src/core/model.ts:1086`
...
### 函数 `clearImageSourceDefault`

源码：`src/core/model.ts:1110`
...
### 函数 `setImageSourceDefault`

源码：`src/core/model.ts:1122`
...
### 函数 `migrateLocalAssetBlocks`

源码：`src/core/model.ts:1229`

遍历文档节点内容块，把本地图片块与上传文件块迁移到新仓库路径并改写权威引用。 纯逻辑函数，不执行任何文件系统操作；`resolveNewPath` 由调用方实现，负责根据 旧本地路径返回迁移后的新路径（返回 null 表示无需迁移）。图片块同时改写 `localSource`，并在 `source` 仍指向旧本地路径时一并改写；上传文件块改写 `source`。远程图床 URL 与 `remoteSources` 保持原样，不参与迁移。

```ts
export function migrateLocalAssetBlocks( root: MindMapNode, resolveNewPath: (oldPath: string) => string | null ): number
```

### 函数 `applyImageUploadPatches`

源码：`src/core/model.ts:1274`
...
### 函数 `buildNodeTreeIndex`

源码：`src/core/node-tree.ts:25`

Builds one depth-first snapshot for repeated node, parent and traversal lookups. The index stores live node references, so callers must rebuild it after changing parent/child relationships; content-only edits do not invalidate the snapshot.
...
### 函数 `indexedHasAncestor`

源码：`src/core/node-tree.ts:59`
...
### 函数 `indexedHasAnyAncestor`

源码：`src/core/node-tree.ts:70`
...
源码：`src/core/node-tree.ts:152`

将节点移动到目标节点之前、之后或内部。

```ts
export function moveNodeRelative( root: MindMapNode, draggedId: string, targetId: string, position: NodeDropPosition, existingIndex?: NodeTreeIndex ): boolean
```
...
统一“主题与外观”弹窗：画布、节点、连线、文章编号、代码与阅读样式设置。

### 接口 `ArticleNumberingValues`

源码：`src/editor/appearance-modal.ts:24`
...
源码：`src/editor/appearance-modal.ts:30`
...
源码：`src/editor/appearance-modal.ts:35`
...
源码：`src/editor/appearance-modal.ts:47`
...
源码：`src/editor/appearance-modal.ts:59`
...
源码：`src/editor/appearance-modal.ts:231`
...
```ts
export function createArticleNumberingControls( container: HTMLElement, currentMode: ArticleNumberingMode | undefined, currentLevel: number | undefined, onChange?: () => void ): ArticleNumberingControls
```

### 类 `AppearanceModal`

源码：`src/editor/appearance-modal.ts:282`
...
源码：`src/editor/appearance-modal.ts:306`
...
源码：`src/editor/appearance-modal.ts:337`
...
源码：`src/editor/article-renderer.ts:79`
...
源码：`src/editor/article-renderer.ts:107`
...
源码：`src/editor/article-renderer.ts:138`
...
源码：`src/editor/article-renderer.ts:340`
...
源码：`src/editor/article-renderer.ts:353`
...
源码：`src/editor/article-renderer.ts:358`
...
源码：`src/editor/article-renderer.ts:365`
...
源码：`src/editor/article-renderer.ts:449`
...
源码：`src/editor/article-renderer.ts:472`
...
源码：`src/editor/article-renderer.ts:617`
...
源码：`src/editor/article-renderer.ts:622`
...
源码：`src/editor/article-renderer.ts:636`
...
源码：`src/editor/article-renderer.ts:676`
...
```ts
export function canMoveNodes( root: MindMapNode, selectedIds: ReadonlySet<string>, draggedId: string | null, targetId: string, existingIndex?: NodeTreeIndex ): boolean
```

### 函数 `resolveDropPosition`

源码：`src/editor/drag-drop.ts:44`
...
### 类型 `ImagePreviewSourceChange`

源码：`src/editor/editor-modals.ts:120`

图片预览弹窗内的一次来源变更请求。 `reupload` 表示选择本地图片并上传图床（图床与手动 URL 来源）；`replaceLocal` 表示选择本地 图片替换本地图片；`remove` 删除一个来源（没有任何剩余来源时由宿主删除整个图片块）； `add` 把用户手动填写的 URL 添加为新来源；`setDefault` / `unsetDefault` 写入或取消图片级来源优先级。

```ts
export type ImagePreviewSourceChange = |
```

### 接口 `ImagePreviewSourceActions`

源码：`src/editor/editor-modals.ts:134`

图片预览弹窗来源管理动作，由编辑器注入并走统一历史与保存链路。 `applyChange` 返回 true 表示图片块仍然存在（弹窗保持打开并刷新来源栏）， 返回 false 表示图片块已被删除（弹窗应关闭）。
...
源码：`src/editor/editor-modals.ts:148`
...
源码：`src/editor/editor-modals.ts:163`

创建图片预览弹窗。

```ts
constructor( app: App, private readonly source: string, private readonly alt: string, private readonly sources: MindMapImageSourceCandidate[] = [], private readonly resolveSource?: (source: string) => string | null, private readonly actions?: ImagePreviewSo…
```

### 方法 `ImagePreviewModal.runSourceChange`
...
源码：`src/editor/editor-modals.ts:434`
...
源码：`src/editor/editor-modals.ts:442`
...
源码：`src/editor/editor-modals.ts:461`
...
源码：`src/editor/editor-types.ts:45`
...
源码：`src/editor/editor-types.ts:99`
...
### 接口 `ToolbarAvailabilityContext`

源码：`src/editor/editor.ts:175`
...
### 方法 `MindMapEditor.setArticleContextLoadingProgress`

源码：`src/editor/editor.ts:403`
...
### 方法 `MindMapEditor.renderArticleContextLoadingProgress`

源码：`src/editor/editor.ts:425`

根据当前显示模式与文章族解析状态，挂载、更新或移除右下角加载进度浮层。 该浮层仅提供只读反馈，不参与正文首帧门、滚动恢复或文档持久化。
...
### 方法 `MindMapEditor.applyRecoveredNavigation`

源码：`src/editor/editor.ts:550`

把视图层从父节点 `submap.path` 反查出的父级导航应用到当前运行态。 这里只刷新导航控件，不进入撤销历史、不触发保存，也不重绘画布节点。
...
### 方法 `MindMapEditor.captureArticleRebuildLocation`

源码：`src/editor/editor.ts:1026`

文章族内容刷新会先渲染骨架并清空正文，滚动位置随之丢失。刷新前把当前语义位置 暂存下来，供骨架之后的那次真实渲染恢复：只要骨架前的正文还在，就记录 35% 视口 扫描线所在的节点与比例，避免内容变更（例如在图片前添加文字）后视口跳到别处。
...
### 方法 `MindMapEditor.invalidateDocumentSnapshotJson`

源码：`src/editor/editor.ts:1318`
...
### 方法 `MindMapEditor.assertDocumentSnapshotCacheFresh`

源码：`src/editor/editor.ts:1329`

低频抽样校验已缓存的修订序列化与当前文档完全一致。 任何绕过统一失效入口的持久字段写入都会在这里被发现并自愈：缓存立即失效， 下一次读取回到完整序列化，防止过期 JSON 进入撤销栈或宿主保存链路。 间隔采样避免大型导图频繁承担全量序列化成本。

```ts
private assertDocumentSnapshotCacheFresh(): void
```

### 方法 `MindMapEditor.currentDocumentSnapshotJson`

源码：`src/editor/editor.ts:1341`
...
### 方法 `MindMapEditor.captureHistorySnapshot`
...
### 方法 `MindMapEditor.createDetachedDocumentSnapshot`

源码：`src/editor/editor.ts:1360`
...
### 方法 `MindMapEditor.getPersistedViewState`

源码：`src/editor/editor.ts:1378`
...
### 方法 `MindMapEditor.notifyDocumentChange`

源码：`src/editor/editor.ts:1384`
...
### 方法 `MindMapEditor.collectFileAssetPaths`

源码：`src/editor/editor.ts:1392`
...
### 方法 `MindMapEditor.collectDeletedFileAssetPaths`

源码：`src/editor/editor.ts:1404`
...
### 方法 `MindMapEditor.toolbarAvailabilityContext`

源码：`src/editor/editor.ts:2219`

Computes tree-dependent toolbar state in one traversal so each toolbar item does not independently rescan a large document.
...
源码：`src/editor/editor.ts:2241`

Returns whether one configured toolbar action can perform a meaningful operation now.

```ts
private toolbarItemAvailable( id: ToolbarItemId, context: ToolbarAvailabilityContext ): boolean
```

### 方法 `MindMapEditor.updateToolbarAvailability`

源码：`src/editor/editor.ts:2284`
...
```ts
private reapplyArticleAnchor(): void
```

### 方法 `MindMapEditor.holdArticleAnchor`

源码：`src/editor/editor.ts:3440`
...
源码：`src/editor/editor.ts:5706`

将图片 Blob 落盘并作为内容块插入目标节点，右键选图、粘贴图片与截图插入共用。 走统一 mutate 链路（撤销、保存、阅读位置记忆），并按节点当前状态排程图床自动上传。

```ts
private async insertImageBlockToNode( nodeId: string, blob: Blob, filename: string, alt: string | undefined, label: string, afterBlockId?: string ): Promise<MindMapImageContentBlock | null>
```

### 方法 `MindMapEditor.insertImageToNode`

源码：`src/editor/editor.ts:5760`
...
源码：`src/editor/editor.ts:6754`

按节点与内容块 ID 定位图片块；不存在时返回 null。

```ts
private locateImageBlock(nodeId: string, blockId: string):
```

### 方法 `MindMapEditor.commitImagePreviewSourceChange`

源码：`src/editor/editor.ts:6771`

提交图片预览中的来源元数据修改，但不重建整个当前视图。来源变化不改变节点拓扑， 因此保留统一历史/保存链路后只刷新对应图片块即可，避免文章 DOM、其它图片和滚动进度失效。

```ts
private commitImagePreviewSourceChange( nodeId: string, blockId: string, action: () => void, previousSnapshot?: string ): void
```
...
源码：`src/editor/editor.ts:7849`

所有用户可撤销写操作的统一入口。调用前克隆当前文档写入撤销栈，执行修改并重渲染，再按影响级别通知视图保存和刷新文章上下文。

```ts
private mutate( action: () => void, restoreLocation?: ReadingLocation | null, articleContextImpact: ArticleContextChangeImpact = "structure" ): void
```

### 方法 `MindMapEditor.undo`

源码：`src/editor/editor.ts:7873`
...
## `src/editor/file-block-view.ts`

文件内容块卡片的共享渲染：导图画布、文章、大纲与节点编辑器复用同一 DOM 结构。

### 接口 `FileCardOptions`

源码：`src/editor/file-block-view.ts:11`
...
### 函数 `fileCardSizeLabel`

源码：`src/editor/file-block-view.ts:26`
...
源码：`src/editor/file-block-view.ts:43`
...
### 方法 `DocumentHistory.captureSnapshot`

源码：`src/editor/history-manager.ts:50`
...
源码：`src/editor/history-manager.ts:61`
...
### 方法 `DocumentHistory.undoSnapshot`

源码：`src/editor/history-manager.ts:72`
...
### 方法 `DocumentHistory.redoSnapshot`

源码：`src/editor/history-manager.ts:95`
...
源码：`src/editor/history-manager.ts:104`
...
### 方法 `DocumentHistory.createSnapshot`

源码：`src/editor/history-manager.ts:115`
...
### 方法 `DocumentHistory.restoreSnapshot`

源码：`src/editor/history-manager.ts:125`
...
导图节点的 DOM 渲染：内容块、拖拽、上下文菜单与选择行为经上下文对象接入编辑器。

### 接口 `MindMapNodeRendererContext`

源码：`src/editor/mind-map-node-renderer.ts:30`
...
源码：`src/editor/mind-map-node-renderer.ts:78`

渲染单个导图节点及其全部内容块与交互绑定；状态经 `ctx` 读写编辑器。

```ts
export function renderMindMapNode( ctx: MindMapNodeRendererContext, position: LayoutResult["nodes"][number], appearance: MindMapAppearance, branchColorMap: ReadonlyMap<string, string> ): void
```
...
```ts
export function insertSiblingAfter( root: MindMapNode, targetId: string, sibling: MindMapNode, existingIndex?: NodeTreeIndex ): boolean
```

### 函数 `topLevelSelectedNodeIds`

源码：`src/editor/node-actions.ts:40`

从多选集合中过滤掉根节点、无效节点以及已被另一所选祖先覆盖的后代。

```ts
export function topLevelSelectedNodeIds( root: MindMapNode, selectedIds: Iterable<string>, existingIndex?: NodeTreeIndex ): string[]
```

### 函数 `deleteNodes`

源码：`src/editor/node-actions.ts:52`
...
```ts
export function deletionSelectionFallback( root: MindMapNode, ids: Iterable<string>, existingIndex?: NodeTreeIndex ): string
```

### 函数 `setAllBranchesCollapsed`

源码：`src/editor/node-actions.ts:100`
...
节点编辑弹窗：内容块、备注、链接、图标与外观字段的完整编辑界面。

### 函数 `parseDataUrlImageFromHtml`

源码：`src/editor/node-edit-modal.ts:36`

从剪贴板 text/html 中提取首个内嵌 data URI 图片。 Chromium 的 `navigator.clipboard.read()` 只暴露位图白名单格式，网页复制的 SVG（image/svg+xml）仅存在于 text/html 的 `<img src="data:image/...">` 中； 这里解析并解码首个匹配项，避免按钮路径误报“没有可粘贴的图片”。
...
源码：`src/editor/node-edit-modal.ts:56`
...
源码：`src/editor/node-edit-modal.ts:81`
...
源码：`src/editor/node-edit-modal.ts:105`
...
源码：`src/editor/node-edit-modal.ts:126`
...
源码：`src/editor/node-edit-modal.ts:711`
...
源码：`src/editor/node-edit-modal.ts:724`
...
源码：`src/editor/node-image-actions.ts:43`
...
源码：`src/editor/node-image-actions.ts:61`
...
源码：`src/editor/node-image-actions.ts:122`
...
源码：`src/editor/outline-renderer.ts:21`
...
源码：`src/editor/outline-renderer.ts:44`
...
源码：`src/editor/outline-renderer.ts:112`
...
计算节点选择样式需要更新的最小 ID 集合，避免每次选择变化扫描全部 DOM。

### 函数 `selectionClassDelta`

源码：`src/editor/selection-class-delta.ts:16`
...
导图画布视口控制器：持有缩放/平移状态与双指手势，提供变换、适应视图和动画能力。

### 接口 `TouchGestureState`

源码：`src/editor/viewport-controller.ts:6`
...
### 接口 `ViewportLayoutBounds`

源码：`src/editor/viewport-controller.ts:16`
...
### 接口 `ViewportControllerElements`

源码：`src/editor/viewport-controller.ts:24`
...
### 类 `ViewportController`

源码：`src/editor/viewport-controller.ts:36`

持有导图画布的缩放、平移与双指手势状态，并把变换应用到场景元素。 状态经编辑器的存取器转发，交互监听仍由编辑器注册（与选区逻辑交织）， 因此本类只承载可独立推导的变换机制。

```ts
export class ViewportController
```

### 方法 `ViewportController.attach`

源码：`src/editor/viewport-controller.ts:49`
...
### 方法 `ViewportController.applyTransform`

源码：`src/editor/viewport-controller.ts:54`
...
### 方法 `ViewportController.fitToView`

源码：`src/editor/viewport-controller.ts:63`
...
### 方法 `ViewportController.animateViewportTo`

源码：`src/editor/viewport-controller.ts:78`
...
### 方法 `ViewportController.setZoom`

源码：`src/editor/viewport-controller.ts:114`
...
### 方法 `ViewportController.applyZoomInput`

源码：`src/editor/viewport-controller.ts:121`
...
### 方法 `ViewportController.beginTwoFingerGesture`

源码：`src/editor/viewport-controller.ts:132`
...
### 方法 `ViewportController.updateTwoFingerGesture`

源码：`src/editor/viewport-controller.ts:146`
...
### 方法 `ViewportController.clampZoom`

源码：`src/editor/viewport-controller.ts:179`
...
源码：`src/import/import-export.ts:25`
...
源码：`src/import/import-export.ts:42`
...
### 接口 `XMindEmbeddedImage`

源码：`src/import/import-export.ts:49`
...
### 接口 `XMindImportResult`

源码：`src/import/import-export.ts:58`
...
### 接口 `XMindImageMaterializeResult`
...
### 接口 `XMindImageCandidate`

源码：`src/import/import-export.ts:73`
...
源码：`src/import/import-export.ts:85`
...
源码：`src/import/import-export.ts:90`
...
```ts
function walkXMindMetadata( value: unknown, visitor: (key: string, value: unknown) => void, depth = 0 ): void
```

### 函数 `collectXMindImages`

源码：`src/import/import-export.ts:153`
...
### 函数 `xmindEquationSource`

源码：`src/import/import-export.ts:183`
...
### 函数 `collectXMindEquationValues`

源码：`src/import/import-export.ts:191`
...
### 函数 `collectXMindEquations`

源码：`src/import/import-export.ts:211`
...
### 函数 `xmindResourcePath`

源码：`src/import/import-export.ts:220`
...
### 函数 `xmindArchiveEntry`

源码：`src/import/import-export.ts:234`

Resolves an XMind image reference against archive entries using tolerant path matching.

```ts
function xmindArchiveEntry( archive: Record<string, Uint8Array>, lowerPaths: Map<string, string>, source: string ):
```

### 函数 `xmindImageMimeType`

源码：`src/import/import-export.ts:253`
...
### 函数 `xmindImageFilename`

源码：`src/import/import-export.ts:270`
...
源码：`src/import/import-export.ts:283`
...
### 函数 `rewriteXMindImageTokens`

源码：`src/import/import-export.ts:300`
...
源码：`src/import/import-export.ts:328`

Merges a linked XMind sheet root into its mount topic. XMind topic links are nested-sheet portals. The mount topic already owns the visible title, even when the linked sheet root was renamed independently, so only the linked root's additional content blocks, notes, and children are merged. This prevents an empty portal and a second, content-bearing root node from being imported side by side.
...
源码：`src/import/import-export.ts:366`
...
```ts
export async function materializeXMindImages( result: XMindImportResult, saveImage: (image: XMindEmbeddedImage) => Promise<string> ): Promise<XMindImageMaterializeResult>
```
...
```ts
async recognizeImage( image: RecognizableImage, blob: Blob, profileId?: string, instruction?: string, remoteUrl?: string, signal?: AbortSignal ): Promise<ImageRecognitionItemResult>
```
...
### 方法 `MindMapStudioPlugin.mindMapFileRevision`
...
### 方法 `MindMapStudioPlugin.resolveMindMapFileRevision`
...
### 方法 `MindMapStudioPlugin.getCachedMindMapDocument`
...
### 方法 `MindMapStudioPlugin.recoverSubmapNavigation`

源码：`src/main.ts:1504`

为缺失 `navigation.parentPath` 的旧子导图恢复父级导航。 先等待全局搜索索引完成启动时的增量校验，再从已校验条目中反查父级挂载关系。 恢复关系必须来自父节点真实的 `submap.path`，因此不会把普通节点链接当成父导图。

```ts
async recoverSubmapNavigation(file: TFile, document: MindMapDocument): Promise<MindMapNavigation | null>
```

### 方法 `MindMapStudioPlugin.rememberMindMapDocument`
...
### 方法 `MindMapStudioPlugin.getMindMapCacheRevision`
...
### 方法 `MindMapStudioPlugin.getCachedArticleContext`

源码：`src/main.ts:1537`

同步恢复仍与全部父子文件版本一致的文章上下文。当前物理页始终替换为刚加载的文档， 因此缓存只复用跨文件计算结果，不会覆盖 TextFileView 本次收到的数据。

```ts
getCachedArticleContext(file: TFile, currentDocument: MindMapDocument): ArticleContextData | null
```

### 方法 `MindMapStudioPlugin.cacheArticleContext`

源码：`src/main.ts:1554`

保存一次成功构建的文章上下文。若构建期间任何 .mindmap 发生过变化，则放弃本次快照， 避免旧文档内容与新的 mtime/size 被错误配对。

```ts
cacheArticleContext(file: TFile, context: ArticleContextData, buildRevision: number): boolean
```

### 方法 `MindMapStudioPlugin.invalidateMindMapCaches`

源码：`src/main.ts:1584`

让一个物理导图及所有依赖它的文章族缓存失效。create/rename 可选择清空全部上下文， 因为新路径可能让旧快照中未记录的“缺失子导图”引用变为可解析。
...
### 方法 `MindMapStudioPlugin.saveAttachmentFile`
...
### 方法 `MindMapStudioPlugin.scheduleFileAssetDeletion`

源码：`src/main.ts:2096`

为已删除引用的文件块附件登记 60 秒延迟回收任务。 同一路径重复调度只保留首个定时器；到期执行时仍有全库引用检查兜底， 期间任何文档变化（撤销、粘贴恢复等）都会经 cancelFileAssetDeletion 取消任务。

```ts
scheduleFileAssetDeletion(paths: string[], currentMindMapPath: string): void
```

### 方法 `MindMapStudioPlugin.cancelFileAssetDeletion`
...
### 方法 `MindMapStudioPlugin.deleteFileAssetIfSafe`

源码：`src/main.ts:2132`

在删除文件块附件前进行最终安全检查：文件必须存在于仓库内，且没有任何 .mindmap 文档仍然引用该路径。 引用检查同时覆盖文件块 source 与图片块本地副本 localSource。

```ts
private async deleteFileAssetIfSafe(localPath: string, currentMindMapPath: string): Promise<boolean>
```

### 方法 `MindMapStudioPlugin.revealFileInSystemExplorer`

源码：`src/main.ts:2172`

在系统文件资源管理器中显示仓库内文件并聚焦选中（桌面端，行为同 VS Code 的 Reveal in Explorer）。 移动端或 Electron API 不可用时提示不支持；文件不存在时先给出明确提示。

```ts
async revealFileInSystemExplorer(vaultPath: string): Promise<void>
```

### 方法 `MindMapStudioPlugin.openFileAsset`
...
将指定节点及其后代提取为独立子导图文件。 提取时会同步把该子树引用的本地图片与上传文件块迁移到子导图自己的资源目录并改写引用， 使子导图完全独立；父导图子树被移除后，原附件不再被引用，登记 60 秒延迟安全回收， 避免与子导图副本重复并存。

```ts
async extractToSubmap(parentFile: TFile, node: MindMapNode): Promise<MindMapSubmap>
```

### 方法 `MindMapStudioPlugin.migrateSubmapAssets`

源码：`src/main.ts:3449`

将文档内容块引用的本地图片块与上传文件块迁移到目标导图自己的资源目录，并改写引用。 子导图提取（复制保留原图）或合并回父导图（复制并按需回收原图）时调用， 保证子导图与父导图各自自包含、相互独立。远程图床 URL 不迁移。

```ts
private async migrateSubmapAssets( document: MindMapDocument, targetFile: TFile, deleteOriginals: boolean ): Promise<string[]>
```
...
### 方法 `MindMapStudioPlugin.cleanupEmptySubmapAssetsFolder`

源码：`src/main.ts:3559`

合并回父导图后，子导图引用的附件已被迁出并回收；若其资源目录因此变成空目录， 主动删除。嵌套场景下（子导图被保存在主图资产目录内的子目录等）会沿空目录链向上 继续清理，保留主导图自己的资源根目录以免误删。
...
源码：`src/search/global-search.ts:22`
...
源码：`src/search/global-search.ts:46`
...
源码：`src/search/global-search.ts:55`
...
源码：`src/search/global-search.ts:66`
...
源码：`src/search/global-search.ts:75`
...
源码：`src/search/global-search.ts:89`
...
源码：`src/search/global-search.ts:100`
...
源码：`src/search/global-search.ts:112`
...
源码：`src/search/global-search.ts:136`
...
源码：`src/search/global-search.ts:173`
...
源码：`src/search/global-search.ts:187`
...
源码：`src/search/global-search.ts:255`
...
源码：`src/search/global-search.ts:281`
...
源码：`src/search/global-search.ts:325`
...
源码：`src/search/global-search.ts:352`
...
源码：`src/search/global-search.ts:367`
...
源码：`src/search/global-search.ts:376`
...
源码：`src/search/global-search.ts:384`
...
源码：`src/search/global-search.ts:395`
...
源码：`src/search/global-search.ts:407`
...
源码：`src/search/global-search.ts:441`
...
### 方法 `MindMapSearchIndex.findParentNavigationForChild`

源码：`src/search/global-search.ts:452`

从已完成版本校验的搜索索引中反查当前子导图的父级挂载节点。 仅接受父节点 `submap.path` 实际解析到目标子文件的关系，避免把普通链接误判为父子导航。

```ts
findParentNavigationForChild(childPath: string): MindMapNavigation | null
```

### 方法 `MindMapSearchIndex.isIndexedFileFresh`

源码：`src/search/global-search.ts:478`
...
### 方法 `MindMapSearchIndex.indexFamilyDocument`

源码：`src/search/global-search.ts:489`
...
### 方法 `MindMapSearchIndex.familyIndexedFile`

源码：`src/search/global-search.ts:509`

获取导图族遍历所需的单文件索引。新鲜索引直接复用；只有缺失或过期时才读取并解析文件。 同一次族刷新内的解析结果会写入 documents，避免父级爬升和后续向下遍历重复读取同一文件。

```ts
private async familyIndexedFile( file: TFile, documents: Map<string, MindMapDocument> ): Promise<IndexedMindMapFile | null>
```

### 方法 `MindMapSearchIndex.refreshFamily`

源码：`src/search/global-search.ts:539`

Refresh a parent map and every recursively linked child map, then return the exact set of files that belongs to that map family. Fresh search-index snapshots are reused so opening family search does not re-read every parent and child file when nothing changed on disk.
...
### 方法 `MindMapSearchIndex.refreshFile`

源码：`src/search/global-search.ts:622`

立即重建单个导图文件的搜索条目并持久化索引。批量替换等已知写入流程 使用该入口，避免等待文件监听器的延迟队列而继续显示旧结果。
...
源码：`src/search/global-search.ts:639`
...
源码：`src/search/global-search.ts:653`
...
源码：`src/search/global-search.ts:661`
...
源码：`src/search/global-search.ts:670`
...
源码：`src/search/global-search.ts:682`
...
源码：`src/search/global-search.ts:711`
...
源码：`src/search/global-search.ts:738`
...
源码：`src/search/global-search.ts:752`
...
源码：`src/search/global-search.ts:780`
...
源码：`src/search/global-search.ts:791`
...
源码：`src/search/global-search.ts:807`
...
源码：`src/search/global-search.ts:849`
...
源码：`src/search/global-search.ts:874`
...
源码：`src/search/global-search.ts:890`
...
源码：`src/search/global-search.ts:897`
...
源码：`src/search/global-search.ts:1001`
...
源码：`src/search/global-search.ts:1015`
...
源码：`src/search/global-search.ts:1060`
...
源码：`src/search/global-search.ts:1120`
...
源码：`src/search/global-search.ts:1131`
...
源码：`src/search/global-search.ts:1146`
...
源码：`src/search/global-search.ts:1166`
...
源码：`src/search/global-search.ts:1175`
...
源码：`src/utils/desktop-capture.ts:444`
...
源码：`src/utils/desktop-capture.ts:456`
...
源码：`src/utils/desktop-capture.ts:478`
...
源码：`src/utils/desktop-capture.ts:498`
...
源码：`src/utils/desktop-capture.ts:619`
...
源码：`src/utils/desktop-capture.ts:646`
...
源码：`src/utils/desktop-capture.ts:667`
...
源码：`src/utils/desktop-capture.ts:759`
...
源码：`src/utils/desktop-capture.ts:806`
...
源码：`src/utils/desktop-capture.ts:820`
...
源码：`src/utils/desktop-capture.ts:912`
...
### 函数 `remoteImageSuggestedName`

源码：`src/utils/filename.ts:100`
...
源码：`src/utils/image-host.ts:124`
...
源码：`src/utils/image-host.ts:135`
...
源码：`src/utils/image-host.ts:149`
...
源码：`src/utils/image-host.ts:174`
...
源码：`src/utils/image-host.ts:202`
...
源码：`src/utils/image-host.ts:223`
...
源码：`src/utils/image-host.ts:236`
...
源码：`src/utils/image-host.ts:252`
...
源码：`src/utils/image-host.ts:258`
...
### 方法 `MindMapStudioView.recoverMissingSubmapNavigation`

源码：`src/view.ts:332`

异步恢复旧子导图缺失的父级导航，并只刷新受影响的导航控件与文章上下文。 该兼容修复不会把文件标记为已编辑；用户之后产生真实内容修改时，恢复出的导航会随正常保存一并持久化。
...
### 方法 `MindMapStudioView.currentDocumentSnapshot`

源码：`src/view.ts:379`

Reuses the detached document snapshot received from the editor change callback. Only viewport metadata can change without a document mutation, so persistence merges that small object instead of cloning the complete node tree again.
...
### 方法 `MindMapStudioView.openMapFamilySearchFromShortcut`
...
### 方法 `MindMapStudioView.scheduleArticleContentContextRefresh`
...
### 方法 `MindMapStudioView.syncCurrentReadingSection`
...
### 方法 `MindMapStudioView.refreshArticleContentContext`
