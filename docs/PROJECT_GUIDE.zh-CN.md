...
桌面端由 `src/utils/desktop-capture.ts` 优先尝试渲染器可用的 Electron `desktopCapturer`，3.5 秒内不可用时改用本机非交互式命令抓取鼠标所在显示器，本机路径最长等待 18 秒。抓取完成后不再异步调用可能被 Obsidian 拦截的 `window.open`，而是直接把全屏 `iframe` 挂载到当前 Obsidian 文档根节点，并通过临时 Blob URL显示截图。选区支持拖动、八方向缩放，并实时显示全局 `X / Y / 宽 × 高`。普通截图保持选区常驻，双击选区后复制并插入节点；截图并识别在拖拽选区或调整边框释放后等待 3 秒自动完成，选区内鼠标活动或继续调整边框会重置等待时间。底部工具栏提供几何图形、画笔、箭头、文字、序号、马赛克、橡皮擦、识别并复制、固定、下载、取消和复制。点击命令时先显示准备提示；禁止静默回退到操作系统交互式截图工具，抓取或覆盖层创建失败时显示明确错误并记录诊断阶段。
