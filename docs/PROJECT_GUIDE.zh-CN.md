...
桌面端优先由 `src/utils/desktop-capture.ts` 使用 Electron 屏幕源打开无边框截图覆盖层。选区支持拖动、八方向缩放，并实时显示全局 `X / Y / 宽 × 高`。底部工具栏提供几何图形、画笔、箭头、文字、序号、马赛克、橡皮擦、识别并复制、固定、下载、取消和复制。覆盖层不可用时回退到操作系统截图工具。
