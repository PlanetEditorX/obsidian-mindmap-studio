...
桌面端由 `src/utils/desktop-capture.ts` 负责抓屏与编辑。Windows 优先使用启用 Per-Monitor V2 DPI 感知的 PowerShell/WinForms 路径抓取完整虚拟桌面，并记录全部显示器边界；编辑器在当前 Obsidian 根节点挂载全屏 `iframe`，通过“全部屏幕 / 屏幕 N”切换查看任意显示器。截图背景按比例适配而不拉伸，选区与导出使用虚拟桌面原始像素映射。选区支持拖动、八方向缩放，并实时显示全局 `X / Y / 宽 × 高`。普通截图保持选区常驻，双击选区后复制并插入节点；截图并识别在拖拽选区或调整边框释放后等待 3 秒自动完成，选区内鼠标活动或继续调整边框会重置等待时间。工具栏提供几何图形、画笔、箭头、文字、序号、马赛克、橡皮擦、识别并复制、固定、下载、取消和复制；几何图形、画笔、箭头、文字和序号支持颜色、粗细及形状样式，箭头可切换为直线；文字输入框使用延迟稳定聚焦并兼容中文输入法；显示器名称由编辑器本地生成，避免 PowerShell 编码乱码；Windows 固定截图使用带就绪握手和错误回传的真正置顶 WinForms 窗口。点击命令时先显示准备提示；禁止静默回退到操作系统交互式截图工具，抓取或覆盖层创建失败时显示明确错误并记录诊断阶段。
