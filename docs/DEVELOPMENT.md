...
LaTeX 维护规则：模型层只保存 `$...$` 与 `$$...$$` 源码，不保存 MathJax 输出 DOM。文章行内编辑初始化时必须保留查看态公式；只有真实聚焦后才切换到源码，失焦无论内容是否变化都要立刻调用 `renderRichTextRuns(..., latex=true)`。异步 MathJax 回调在容器仍为活动 `contenteditable` 时必须放弃重绘。题目编辑器的题干、全部选项、答案和解答应共用同一右键插入与预览函数，新增题目字段时不得遗漏公式能力。 `$...$` 行内公式要同时检查 `renderMath(..., false)` 返回的公式外层和内部 `mjx-container`；仅把外层设为 `inline-block` 不足以阻止 Obsidian/MathJax 内层块级宽度换行。题目 AI 处理的流式回调只展示接口实际返回的 reasoning/content，阶段状态必须在不支持思考字段的兼容模型上仍可用；最终结构化 JSON 仍需经过解析校验后才能回填。
