...
- 文档快照边界保持“编辑器内部模型可变、宿主快照隔离”：真实 mutation 只允许在 `notifyDocumentChange()` 边界生成一次完整 `getDocument()` 克隆；View 同一修订的保存、文章上下文、AI/识图和搜索必须复用收到的宿主快照，不得为了只读消费再次深拷贝整棵树。无 mutation 的 zoom/pan 持久化只通过 `getPersistedViewState()` 合并轻量 `view` 对象。若未来需要让宿主修改文档，必须通过明确编辑器 API 回写，不能把内部 document 引用直接暴露给 View。
