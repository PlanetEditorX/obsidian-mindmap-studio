...
LaTeX 维护规则：模型层只保存 `$...$` 与 `$$...$$` 源码，不保存 MathJax 输出 DOM。文章行内编辑初始化时必须保留查看态公式；只有真实聚焦后才切换到源码，失焦无论内容是否变化都要立刻调用 `renderRichTextRuns(..., latex=true)`。异步 MathJax 回调在容器仍为活动 `contenteditable` 时必须放弃重绘。题目编辑器的题干、全部选项、答案和解答应共用同一右键插入与预览函数，新增题目字段时不得遗漏公式能力。
