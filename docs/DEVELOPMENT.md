...
- 同名图片重新识别问题分两步检查：显示层看 `MindMapView.resolveImage()` 的兜底是否生效，持久化看 `MindMapView.relinkMissingImageFiles()` 是否只在打开导图时写回一次。匹配规则与遍历在 `src/core/image-relink.ts`，必须保持纯逻辑：只匹配同名主干且扩展名不同的图片文件，同目录优先、其次全库，原文件仍存在时不得改写。写回必须走 `replaceNodeContentBlocks()` 并保持“不重绘”，否则会扰动文章补载与阅读位置恢复；不要在渲染路径里直接修改文档。
...
LaTeX 维护规则：模型层只保存 `$...$` 与 `$$...$$` 源码，不保存 MathJax 输出 DOM。文章行内编辑初始化时必须保留查看态公式；只有真实聚焦后才切换到源码，失焦无论内容是否变化都要立刻调用 `renderRichTextRuns(..., latex=true)`。异步 MathJax 回调在容器仍为活动 `contenteditable` 时必须放弃重绘。题目编辑器的题干、全部选项、答案和解答应共用同一右键插入与预览函数，新增题目字段时不得遗漏公式能力。 `$...$` 行内公式要同时检查 `renderMath(..., false)` 返回的公式外层和内部 `mjx-container`；仅把外层设为 `inline-block` 不足以阻止 Obsidian/MathJax 内层块级宽度换行。公式解析必须先合并富文本运行段，不能按单个 run 分别匹配美元分隔符。`$$...$$` 只有在整个文字块没有其他可见文字时才作为独立公式；嵌入说明文字的历史双美元公式按行内恢复。公式编辑器必须剥离输入中已有的美元分隔符，渲染含中文标签的 TeX 时使用临时 `\text{...}` 归一化但不得改写持久化源码。公式编辑器的「方框」按钮把选中内容包进 `\boxed{}`，没有选区时包住整条公式，源码为空时插入空方框并把光标放进花括号内。长行内公式不得把页面顶宽：`wrapLatexForLineBreaks()` 只在顶层 `=` 处断行，且首个顶层 `=` 必须留在首行并写成对齐点（`a & = b \\ & = c`，少于两个顶层等号不换行，避免首行只剩左侧式子；首行漏写 `&` 会让后续等号被排到首行右边界之后、出现大段空档），并必须跳过花括号、转义字符、`\left/\right` 成对定界符与已有 `\begin{}` 环境；渲染层只有在容器宽度已知、非编辑态且不是导图节点（`.mmc-node-text`）时才把超宽公式替换为 `aligned` 多行，断行后无法渲染时保留原公式；判断是否超宽必须先用 `measureUnclampedWidth()` 临时解除 `.mms-node-math` 的 `max-width: 100%`（量完立即还原），否则公式被夹在容器宽度上、永远量不出溢出。题目 AI 处理的流式回调只展示接口实际返回的 reasoning/content，阶段状态必须在不支持思考字段的兼容模型上仍可用；最终结构化 JSON 仍需经过解析校验后才能回填。
