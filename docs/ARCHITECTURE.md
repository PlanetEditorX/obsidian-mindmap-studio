...
- 进入文章模式时，编辑器先让模式按钮和 `aria-busy` 加载态完成一次浏览器绘制，再在 `mms-article-render-stage` 隐藏渲染区调用文章渲染器；已有文章页保持可见并暂时禁止交互，首次进入则按文章容器和当前窗口高度动态生成至少 18 行、横向铺满视图的静态文章骨架。新页面完成后在同一滚动容器内原位替换并淡入，加载提示淡出，避免容器先清空造成白屏和 hover/cursor 闪烁。
