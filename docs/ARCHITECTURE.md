...
- 重建文章 DOM 前保存滚动容器的 `scrollTop`、`scrollLeft` 和旧 `scrollHeight`；临时最小高度阻止浏览器把滚动范围收缩到顶部。`article-renderer.ts` 每批通过 `onProgress` 回调通知编辑器，编辑器优先恢复语义节点和节点内部比例，目标尚未填充时退回旧像素位置。
