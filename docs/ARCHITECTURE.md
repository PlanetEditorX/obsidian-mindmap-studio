...
- `article-render-cache.ts` 提供节点级持久缓存。插件 `onload()` 在注册视图前把最近缓存预载进内存，因此 `TextFileView.setViewData()` 可同步取到快照，不等待磁盘。渲染器以文件路径、缓存版本、文章展示签名和节点指纹判定复用：未变化节点先恢复安全静态 HTML，再分帧绑定当前编辑器实例的文字、图片、表格和题目事件；变化节点正常重建。缓存不保存真实 DOM 或事件监听器，代码块节点因包含异步 Markdown 组件而始终重建。缓存写入采用插件私有 JSON、LRU 容量限制与串行防抖，文件删除和重命名会同步清理或迁移。
