...
- `src/core/latex.ts`：纯函数解析公式分隔符、恢复历史重复美元、判断行内/独立布局，在渲染前把裸露中文标签转换为 MathJax 可识别的 `\text{...}`，并把无法自行折行的长公式（顶层 `=` 等式链，含整体 `\boxed{}` 包裹）改写为 `aligned` 多行源码：首个顶层 `=` 留在首行并作为对齐点（`a & = b \\ & = c`），从第二个起才断行，因此至少要有两个顶层 `=`；花括号、转义字符、`\text{}` 与 `\left/\right` 成对定界符、已有 `\begin{}` 环境都不得被拆开。首行必须写入 `&`：否则首行整段落在 `aligned` 的右对齐列，后续行的 `=` 会被排到首行右边界之后，出现大段空档。
...
- 导图节点挂载后由编辑器维护 `mindMapNodeElements: Map<nodeId, HTMLElement>`，画布级尺寸测量、FLIP、拖拽状态、行内编辑和单节点替换优先查该索引；节点移除或整图重绘必须同步删除/清空索引，避免保留失效 DOM 引用。
- 选择 CSS 同步维护 `appliedSelectionIds` 快照。普通点击、Ctrl/Cmd 多选和框选结束后通过 `selectionClassDelta()` 只处理进入/离开选择的节点；当选择数量跨过“单选/多选”边界时，会额外刷新仍处于选择中的节点以同步 `is-multi-selected`。大纲/文章窗口新增或重建 DOM 会使快照失效，并在下一次同步执行一次完整扫描；隐藏模式不做重复写入。
- 当前编辑器同时维护 `NodeTreeIndex`：`nodes` 保留与旧 `flattenNodes()` 一致的 DFS 顺序，`byId` / `parentById` 保存实时节点引用与直接父节点，`hasCollapsibleNodes` 供工具栏直接读取。完整 `render()` 开始时统一重建；若整个 `MindMapDocument.root` 被替换，首次索引访问会通过根对象身份检查懒重建，因此外部编辑替换、撤销/重做和文件切换不会读取旧树。
- 树索引只缓存“结构关系”，不复制节点内容，因此改标题、样式、备注、图片等纯内容字段无需失效。添加、删除、移动节点会在本次操作完成后进入完整渲染并重建索引；批量拖放的第一步可复用修改前索引完成合法性/父节点查找，后续结构移动会按当前树重新建临时索引，避免使用已经过期的父关系。多选顶层过滤与拖放“不能移入自身后代”的判断均沿 `parentById` 父链执行，而不是对每个候选分支做 DFS。
...
- 文章上下文准备完成后，`renderArticleMode()` 先由 `buildArticleNodeInfo()` 建立轻量章节顺序，再通过 `render-window.ts` 计算目标节点的左闭右开真实窗口。普通 MISS 的首窗预算保持目标前后各约 5 KB；同步恢复过文章上下文缓存时，`articleContextCacheHit` 把首窗预算提升到约 32 KB，减少二次打开时重新从很短片段开始的体感。窗口控制器仍只对当前范围创建真实章节 DOM。
...
- 真实目标窗口挂载并完成语义位置恢复后，编辑器启动 `scheduleArticleWindowWarmup()`：先持续调用 `loadAfter()` 补齐后文，再调用 `loadBefore()` 补齐前文；每个动画帧最多处理 4 个约 5 KB 分块，每批都重绑章节折叠、缩略导航、选中状态与文章块移动 UI，因此页面总高度和右侧缩略导航会自动增长，不依赖用户先滚到当前片段末端。活动 `ReadingLocation` 恢复事务存在时预热会等待，避免与精确落点竞争；用户滚动靠近边缘时原有手动/自动边缘扩展继续作为兜底。
- 向上插入前文时，编辑器记录扩展前后的 `scrollHeight` 差值并补偿 `scrollTop`，保持当前章节在屏幕中的位置；后台预热的前文补载也执行同样补偿并暂时阻止阅读位置采集。交互式边缘扩展先让边缘按钮绘制一帧 `is-loading` 流光，再挂载约 5 KB 真实节点；新节点仅做短时淡入，不遮挡已显示内容。
...
- 编辑器维护 `documentSnapshotJson` 作为“最近一次已发布且仍精确的完整文档 JSON”。普通 mutation 开始前通过 `captureHistorySnapshot()` 把上一修订字符串直接交给 `DocumentHistory.captureSnapshot()`，不再重新 `JSON.stringify()` 修改前整棵节点树；mutation 完成后 `notifyDocumentChange()` 使用 `createDetachedDocumentSnapshot(true)` 强制只序列化一次新修订，并从该字符串恢复一个与内部模型隔离的宿主对象，同时把新字符串留作下一次 history 前置快照。`DocumentHistory` 新增 `createSnapshot()` / `restoreSnapshot()` / `captureSnapshot()` / `undoSnapshot()` / `redoSnapshot()`，但撤销和重做栈仍保存完整 JSON 文档，不改变历史边界或 `.mindmap` 格式。zoom/pan、只读、恢复导航、文章落地状态等可在没有 `onChange` 时修改持久文档字段的路径必须调用 `invalidateDocumentSnapshotJson()`；之后首次读取会安全回退完整序列化，禁止复用过期 JSON。`MindMapDocumentView` 继续把 `onChange` 得到的隔离对象作为当前 `documentChangeRevision` 的权威宿主副本，保存、文章族刷新、AI/图片识别和当前导图族搜索不得再次调用 `editor.getDocument()`。
...
全局搜索与当前导图族搜索共用 `GlobalMindMapSearchModal`。1.45.13 的真实 Windows / Obsidian 1.12.7 日志证明，程序化 `.modal-bg.click()` 虽然能产生一个 `click` 调试事件，但不会像真实用户手势那样关闭宿主 Modal；结果导航已经开始后，搜索层仍要用户再手动点击背景才能消失。因此结果打开时设置 `shouldRestoreSelection=false`，并且只调用一次公开的 `Modal.close()`。关闭路径 **不得** 模拟 `.modal-bg` 的 pointer/click，不得修改/删除 `.modal-container` 或 `.modal-bg`，也不得等待 `onClose()` Promise；只给宿主两个 `requestAnimationFrame` 绘制帧完成 history/focus bookkeeping 后就启动导航。`onClose()` 与关闭请求、导航开始分别写入 `modal-on-close`、`result-close-request` / `result-close-return`、`result-navigation-start` 调试事件，真实桌面端若仍异常可以直接判断宿主是否执行了 Modal 关闭回调。 另外，全局入口 `openGlobalSearch()` 使用 `globalSearchLaunchPending + globalSearchModal.isMounted()` 做单实例去重，避免配置快捷键同时进入插件捕获层和 Obsidian 热键处理链时创建两个叠加 Modal；`openMapFamilySearch()` 不使用该守卫。
...
- 父节点可以直接进入子导图。
- 子导图可以通过左上角悬浮面包屑返回父节点。`MindMapStudioView` 若发现当前文档缺少 `navigation.parentPath`，会调用插件服务层从 `MindMapSearchIndex` 反查父节点：索引条目只有在父节点 `submap.path` 实际解析到当前子文件时才可生成恢复导航。恢复后编辑器仅执行 `renderNavigation()`，同时使旧文章上下文缓存失效并异步重建，不触发 `onChange`、撤销历史或打开即保存。
- 全局搜索可以解析整棵导图族。
- 文章模式可以计算跨文件标题层级和目录。
...
节点折叠按钮位于右侧中部，手动尺寸控制点固定在右下角，避免两个定位操作重叠。默认需按住 Shift 才显示尺寸控制点；修饰键状态同时由键盘、指针事件、窗口失焦和页面可见性同步，避免失焦后遗留缩放光标或拦截普通点击。Ctrl/Cmd（macOS 为 Cmd）专用于多选与框选，避免手势冲突。

折叠节点的后代不参与当前布局，因此展开或折叠后需要重新计算。

编辑画布采用两阶段布局：先使用模型内容估算尺寸并生成初始坐标，节点挂载到 DOM 后再读取浏览器实际计算出的宽高。表格、代码、图片或社区主题造成实际尺寸变化时，`ResizeObserver` 会把实测尺寸交给一次权威 `computeLayout()`；该函数内部完成 `collision-layout.ts` 的碰撞消解、节点映射和画布边界计算，调用方不再重复执行第二轮碰撞或边界重算。碰撞检测会按节点顶部排序；当当前节点与后续第一个节点已经满足最小纵向间距时，后续节点只会更远，因此立即结束当前内层扫描。观察器会缓存每个节点最近一次外框尺寸；选择态、阴影或浏览器重复通知没有改变宽高时不再启动二次布局，避免视口锚点平移被重复累计。
...
3. `editor.ts` 只调用契约，不直接依赖插件主类；自包含弹窗类继续拆分到独立模块（如 `node-edit-modal.ts`、`appearance-modal.ts`），视口手势、行内编辑、题目系统按后续批次继续。
...
工具栏按钮先按用户可见配置和规范化顺序创建。每次刷新先由 `toolbarAvailabilityContext()` 单次遍历节点树，得到当前选择、有效非根选择数量、是否存在可折叠节点和编辑权限；随后所有 `toolbarItemAvailable()` 判断共享这份上下文，避免每个按钮分别调用 `findNode()` / `flattenNodes()` 扫描整棵树。不可用按钮保留 DOM 身份但收缩为零宽并移出键盘顺序，重新可用时恢复；首帧在启用过渡前完成一次过滤，防止整排按钮闪现。截图与截图识别作为不可拆分相邻组，统一“导入与导出”固定在末尾。旧 `json`、`export-document`、`export-svg` 和 `article-style` 只作为加载迁移别名。
