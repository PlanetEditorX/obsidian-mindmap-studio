...
- `article-render-cache.ts` 提供节点级持久缓存。插件 `onload()` 在注册视图前把最近缓存预载进内存，因此 `TextFileView.setViewData()` 可同步取到快照，不等待磁盘。渲染器以规范化文件路径、缓存版本、文章展示签名和节点指纹判定复用：节点指纹只包含当前节点自身字段及其层级、编号、样式等渲染上下文，不包含 `children` 子树，避免深链文档对每个祖先重复序列化后代并减少无关祖先失效；文档摘要直接聚合已生成的节点指纹。未变化节点先恢复安全静态 HTML，再分帧绑定当前编辑器实例的文字、图片、表格和题目事件；变化节点正常重建。缓存不保存真实 DOM 或事件监听器，代码块节点因包含异步 Markdown 组件而始终重建。缓存写入采用插件私有 JSON、正确的 oldest-first 内存 LRU、容量限制与串行防抖，文件删除和重命名会同步清理或迁移。
