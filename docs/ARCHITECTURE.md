...
- 进入文章模式时，编辑器先让模式按钮和 `aria-busy` 加载态完成一次浏览器绘制，再在 `mms-article-render-stage` 隐藏渲染区生成标题、占位章节和首批正文；已有文章页在首批内容就绪前保持可见，首次进入则按文章容器和当前窗口高度动态生成至少 18 行、横向铺满视图的静态文章骨架。首批正文完成后页面立即移入同一滚动容器并开放阅读，剩余章节继续按帧替换占位区；全部完成后再安装依赖完整 DOM 的折叠、缩略导航和阅读位置收尾逻辑。
- 重建文章 DOM 前保存滚动容器的 `scrollTop`、`scrollLeft` 和旧 `scrollHeight`；保留页维持旧滚动范围，`article-renderer.ts` 每批通过 `onProgress` 回调通知编辑器维持语义或像素锚点。若用户在渐进加载期间产生滚轮、触摸、指针或翻页键输入，编辑器将视口标记为已由用户接管，清除待恢复的旧语义位置；后续批次只维持页面高度，不再写入 `scrollTop`，完成交换时也跳过旧位置恢复。未发生用户导航时仍按原逻辑恢复语义节点和节点内部比例。
- `article-render-cache.ts` 提供节点级持久缓存。插件 `onload()` 在注册视图前把最近缓存预载进内存，因此 `TextFileView.setViewData()` 可同步取到快照，不等待磁盘。渲染器以规范化文件路径、缓存版本、文章展示签名和节点指纹判定复用：节点指纹只包含当前节点自身字段及其层级、编号、样式等渲染上下文，不包含 `children` 子树，避免深链文档对每个祖先重复序列化后代并减少无关祖先失效；文档摘要直接聚合已生成的节点指纹。预扫描阶段只记录可复用缓存条目，不执行 `innerHTML` 恢复；安全静态 HTML 的解析、危险元素/事件属性净化和当前编辑器实例 hydration 与普通节点重建共用同一帧预算，优先处理当前节点关系链，避免大缓存命中时在首屏显示前同步恢复全部离屏章节。单次文章渲染使用 `WeakMap` 复用节点内容块规范化结果，缓存节点 hydration 再用一次 `[data-block-id]` 扫描建立索引，避免每个块重复查询整个章节 DOM。缓存不保存真实 DOM 或事件监听器，代码块节点因包含异步 Markdown 组件而始终重建。缓存写入采用插件私有 JSON、正确的 oldest-first 内存 LRU、容量限制与串行防抖，文件删除和重命名会同步清理或迁移。只有缓存命中改变 LRU 顺序时才标记访问状态待写，已是最新条目的重复命中不会产生额外磁盘操作；活动写入期间的全部变化合并到下一份稳定 JSON 快照，干净卸载不会重写缓存文件。
...
- 同级编号按有效文章层级分别计数，目录、正文、通读和导出不得各自实现另一套规则。标题编号样式只覆盖 1–8 级；更深结构继续参与标题树和目录深度计算，但不再循环复用 `A.` / `（A）`。第 7、8 级同级数量超过 26 时使用 AA、AB 等无冲突字母序号。末端正文达到阈值后可选择沿用下一层标题编号，或使用独立的带圈数字：`circledNumberLabel()` 仍为纯文本输出提供 1–50 的 Unicode `①–㊿` 与 51+ 可读回退；文章、通读和 HTML 从 `leafNumberingIndex` 取十进制数字并对全部序号使用同一 CSS 圆环，避免操作系统替换字形造成尺寸和基线差异。该样式不受八级标题编号边界限制。
- 所有正文段落统一使用 `2em` 首行缩进，不根据当前窗口下是否换行动态改变；标题和备注不缩进。代码、表格和图片不改变自身内容缩进，但其内容块整体左边缘与正文首行对齐，避免混排时出现中间过宽的视觉轮廓。文章正文内粘贴图片会先提交正在编辑的文字块，再将图片作为该块之后的有序内容块保存，避免浏览器临时图片在重绘后消失。
- 只有存在非空正文文字时才创建正文段落，纯表格、代码或图片节点不会显示“正文段落”占位。
- 文章模式沿用原有入口条件：顶层脑图存在子导图时生成递归目录页；工具栏在纯目录页与原始文章之间切换，原始文章正文不内嵌目录。全局“进入文章模式”策略默认为锁定，也可设为沿用切换前状态或恢复文章模式自己上次的锁定/编辑状态。`remember` 策略读取插件级 `articleLastReadOnly`，文章内切换锁时单独保存，不写回文档 `view.readOnly`，因此不会被导图、大纲、通读或其他文件级锁状态覆盖。
...
`Ctrl/Cmd+Alt+F` 搜索当前导图族，`Ctrl/Cmd+F` 保留给 Obsidian 默认搜索，`Ctrl/Cmd+Shift+F` 打开整个仓库的导图搜索。全局与导图族搜索都只索引和显示节点文字，不匹配或展示子导图路径、备注、标签、链接、代码、表格和图片元数据。导图族快捷键在编辑控件聚焦时也会于编辑器根节点的捕获阶段识别，并使用 `KeyboardEvent.code` 兼容非英文键盘布局。当前导图族搜索会在打开搜索时主动刷新父子链，不要求重新创建子导图。
