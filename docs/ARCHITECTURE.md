...
Obsidian 工作区与仓库
        │
        ▼
main.ts        插件入口、命令、跨文件服务、图床、子导图、迁移
        │
        ▼
view.ts        TextFileView 适配、文件加载保存、文章上下文、资源解析
        │
        ▼
editor.ts      导图/大纲/文章 UI、交互、撤销重做、节点操作
        │
        ├──────────────┬─────────────────┬────────────────┬──────────────────┐
        ▼              ▼                 ▼                ▼                  ▼
editor-types.ts  model.ts         layout.ts          modes.ts       global-search.ts
宿主服务契约      数据模型与兼容    坐标与 SVG          文章编号       本地增量索引
        │
        ▼
.mindmap 原始 JSON 文件
```
...
`Ctrl/Cmd+F` 搜索当前导图族；`Ctrl/Cmd+Shift+F` 搜索整个仓库。当前导图族搜索会在打开搜索时主动刷新父子链，不要求重新创建子导图。
