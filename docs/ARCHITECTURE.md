...
- 同级编号按有效文章层级分别计数，目录、正文、通读和导出不得各自实现另一套规则。标题编号样式只覆盖 1–8 级；更深结构继续参与标题树和目录深度计算，但不再循环复用 `A.` / `（A）`。第 7、8 级同级数量超过 26 时使用 AA、AB 等无冲突字母序号。末端正文达到阈值后可选择沿用下一层标题编号，或使用独立的带圈数字：1–50 由 `circledNumberLabel()` 生成 Unicode `①–㊿`，51 以上保留十进制索引并由文章/HTML CSS 绘圈；该样式不受八级标题编号边界限制。
