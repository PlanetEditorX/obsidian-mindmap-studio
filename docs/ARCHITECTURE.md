...
- 进入文章模式时，编辑器先让模式按钮和 `aria-busy` 加载态完成一次浏览器绘制，再在 `mms-article-render-stage` 隐藏渲染区调用文章渲染器；已有文章页保持可见并暂时禁止交互，首次进入则显示静态文章骨架。新页面完成后在同一滚动容器内原位替换并淡入，加载提示淡出，避免容器先清空造成白屏和 hover/cursor 闪烁。
- 重建文章 DOM 前保存滚动容器的 `scrollTop`、`scrollLeft` 和旧 `scrollHeight`；保留页维持旧滚动范围，`article-renderer.ts` 每批通过 `onProgress` 回调通知编辑器维持语义或像素锚点。完成交换后再次恢复语义节点和节点内部比例，兼容新页面高度变化。
