...
- 编辑器 `notifyDocumentChange()` 仍通过 `getDocument()` 只在变更边界生成一份完整隔离快照，保证 View、插件缓存和异步服务无法反向修改编辑器内部模型。`MindMapDocumentView` 把这份快照作为当前 `documentChangeRevision` 的权威宿主副本；`save()`、`getViewData()`、文章族刷新、AI/图片识别和当前导图族搜索不得再次调用 `editor.getDocument()`。画布缩放和平移是唯一可在没有 mutation 时改变的持久字段，由 `getPersistedViewState()` 只复制 `document.view` 并通过 `currentDocumentSnapshot(true)` 浅合并，因此同一修订的多个宿主消费者不会重复对整棵节点树执行 JSON clone。该优化不改变 `DocumentHistory` 的完整快照撤销语义。
