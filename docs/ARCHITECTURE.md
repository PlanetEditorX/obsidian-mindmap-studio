...
`Ctrl/Cmd+Alt+F` 搜索当前导图族，`Ctrl/Cmd+F` 保留给 Obsidian 默认搜索，`Ctrl/Cmd+Shift+F` 打开整个仓库的导图搜索。导图族快捷键在编辑控件聚焦时也会于编辑器根节点的捕获阶段识别，并使用 `KeyboardEvent.code` 兼容非英文键盘布局。当前导图族搜索会在打开搜索时主动刷新父子链，不要求重新创建子导图。
