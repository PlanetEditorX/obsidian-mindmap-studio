...
本次不修改 `.mindmap` 数据版本。插件设置新增 `readingLocations`，按文章族顶层文件保存目标物理文件、节点祖先链和视口锚点。旧设置无需手工迁移：

- 旧版保存的全局 `outline` 会在插件启动时回到导图模式；导图被隐藏时选择可见的文章或通读模式。大纲仍可在当前会话正常使用和同步。
- 旧 `readingProgress` 百分比继续保留，仅在尚无语义位置时作为兼容后备。
- 第一次在任一模式点击节点或滚动后会生成语义位置。
- 节点删除时恢复到最近仍存在的父级；子导图缺失时恢复到父导图挂载节点。
- `.mindmap` 文件改名时，插件同步迁移 `readingLocations` 和 `readingProgress` 中的路径。

回滚到旧版本不会损坏导图文件；旧版本会忽略新增的插件设置字段。
...
现有文件无需批量迁移。只有用户在节点编辑器或“当前脑图外观”中选择“关闭”或“手动层级（自定义最高层级）”时，才会写入新字段。层级限制为 1–8；中心节点所选层级直接用于一级内容，并同时作用于文章正文、目录、通读、子导图和导出。手动层级不再强制孤立末端节点标题化。
目录结构层级属于运行时派生数据，不写入 `.mindmap` 文件，也无需迁移。升级后，目录最大层级会自动按相对树深度解释，不再受手动编号起始层级影响。

本次新增可选的 `view.articleTocMaxDepth` 字段，用于保存当前脑图对文章/通读目录最大层级的覆盖。旧文件缺少该字段时自动跟随插件全局设置，不需要迁移；只有在“当前脑图外观”中手动选择 1–8 层时才写入，改回“跟随插件设置”后会删除该字段。

导图缩放与平移继续使用既有 `view.zoom`、`view.panX`、`view.panY` 可选字段，不提升数据版本。切换显示模式时会保存并恢复这些字段；旧文件没有视口字段时仍按原设置在首次进入导图时自动适应。
...
文章模式目录交互已恢复为旧版行为：只有顶层脑图包含子导图时才提供纯目录页，原始文章正文不内嵌目录。`view.articleTocMaxDepth` 字段继续保留，并仍优先于插件全局目录最大层级。

文章分页现改为按同层物理子导图兄弟导航，并在子导图页面顶部补充父目录生成的编号。该调整只改变运行时导航和渲染，不新增或修改 `.mindmap` 持久化字段，现有父子导图无需迁移。
