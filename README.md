...
- 图片右键可运行 AI 视觉识图或本机 Tesseract OCR；识别后先并排预览原图与可编辑文字，确认后才在原内容块位置替换。
- 工具栏和 `Ctrl/Cmd+Shift+S` 可启动系统区域截图；可选自动最小化 Obsidian、插入截图前聚焦的节点/段落、仅复制到剪贴板或插入后自动识图。
...
- AI 窗口提供问答、AI 整理并重新生成、图片识图和本地文字替换四种模式；识图按节点树顺序处理当前页面或右键节点子树中的全部图片。
