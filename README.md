...
- 文章行内编辑中，Enter 保存当前内容，Shift+Enter 插入换行；文章编号继承所在标题或正文的字体、字号和行高，圈号编号不会额外放大或加粗。文章编号固定为八级，第 8 级以下的更深结构继续保留标题层级，但不再循环回到 `A.` / `（A）`；同级字母编号超过 Z 后使用 AA、AB。
- 末端正文标识、样式、颜色和对齐方式可分别在插件设置与当前页面“文章样式”中配置；页面设置优先。对齐独立于标识开关，可选择顶格或自动缩进至上级标题下。
- 末端正文达到设置的兄弟节点阈值时，可选择转为“上级标题的下一级文章序号”或“带圈数字”。带圈样式的第 1–50 项使用原生 `①–㊿`，第 51 项以后由 CSS 自动绘制圆圈，因此 60、70 以及更多同级节点都可继续编号；带圈样式也可用于第 8 级标题下面的更深末端正文。默认阈值为 4（3 个节点仍显示标识）。
