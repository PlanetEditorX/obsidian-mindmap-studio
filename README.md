...
- “截图与识别”是位于“AI 助手”之后的独立设置分类。普通截图与截图并识别分别使用 `Ctrl/Cmd+Shift+S` 和 `Ctrl/Cmd+Shift+R`，可独立录制；节点右键和顶部工具栏均提供两种入口。桌面截图编辑器支持可调整选区、全局坐标与尺寸、几何图形、画笔、箭头、文字、序号、马赛克、橡皮擦、识别并复制、固定、下载、取消和复制。普通截图保持选区常驻，双击选区后复制并插入；截图并识别在拖拽选区或调整边框后启动 3 秒自动完成计时，选区内鼠标活动或继续调整边框会重置计时。
