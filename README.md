...
当前源码版本：`1.20.1`。最低 Obsidian 版本：`1.5.0`。
...
- 支持多个 HTTP(S) 图床端点、JSON 请求头、`multipart/form-data` 或原始二进制请求体。
- 可配置响应字段路径，并兼容常见 `data.url`、`result.url` 等结构；粘贴或截图图片默认在本地保留 5 分钟，期间可直接进行 AI 识图，期满后自动上传并保留可达图床 URL。
- 远程镜像与本地源并存；仅在所有选定图床上传成功且确认没有其他引用时，才允许删除本地文件。
- 文件名、扩展名、MIME、请求头和返回 URL 经过独立工具层校验。
- 图片右键可运行 AI 视觉识图或本机 Tesseract OCR；AI 识图可跟随全局接口或单独指定视觉模型，模型角色标记等无效输出会在预览前清理。识别后先并排预览适度缩小且完整可见的原图与可编辑文字，确认后才在原内容块位置替换。
- 工具栏和可配置的编辑器截图快捷键可启动系统区域截图；编辑器默认快捷键为 `Ctrl/Cmd+Shift+S`，可选自动最小化 Obsidian、插入截图前聚焦的节点/段落、仅复制到剪贴板或插入后自动识图。

### AI 助手

- 内置 OpenAI、DeepSeek、硅基流动、FreeLLMAPI 和自定义 OpenAI 兼容接口配置。
- 硅基流动预设提供 DeepSeek V4 Flash、DeepSeek V4 Pro、DeepSeek OCR、PaddleOCR-VL-1.5 和 GLM-5.2 模型建议；FreeLLMAPI 默认使用 `auto` 路由。
