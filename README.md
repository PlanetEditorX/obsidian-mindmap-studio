...
- 粘贴或选择本地图片后写入当前导图附近的资源目录。图片块可设置为“与相邻图片同行”或“独占一行”，同行图片按内容块顺序自动换行；导图、大纲和文章共享该排版。
...
- 远程镜像与本地源并存；节点渲染和点击放大图片会先尝试优先级最高的可用图床地址，失败后再尝试下一个；仅在所有选定图床上传成功且确认没有其他引用时，才允许删除本地文件。图片块被删除后，只有开启“最后引用删除时同步清理图床”、确认全部 `.mindmap` 已无同一 SHA-256/远程 URL 引用，且对应图床配置了删除 API 时，才请求远程删除；未配置删除接口时安全保留图床文件并提示。
