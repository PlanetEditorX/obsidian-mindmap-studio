...
- 文章模式先显示已激活按钮和加载反馈；已有页面会保持可见，新页面在隐藏渲染区按当前节点、兄弟节点、父节点及其兄弟优先填充，完成后原位淡入替换。首次进入则显示静态文章骨架，避免重建期间出现白屏、整页闪烁或鼠标光标在文本与加载状态之间反复切换。
...
- 支持多个 HTTP(S) 图床端点、JSON 请求头、`multipart/form-data` 或原始二进制请求体，并可为图床设置图片加载优先级。上传前会计算图片二进制的 SHA-256；相同图片再次上传到同一图床时复用缓存地址，不重复发送，相同图片上传到不同图床仍分别执行一次。
...
- 远程镜像与本地源并存；节点渲染和点击放大图片会先尝试优先级最高的可用图床地址，失败后再尝试下一个；仅在所有选定图床上传成功且确认没有其他引用时，才允许删除本地文件。图片块被删除后，默认启用“最后引用删除时同步清理图床”；确认全部 `.mindmap` 已无同一 SHA-256/远程 URL 引用，且预设或自定义配置提供了可验证删除接口时，才请求远程删除。Zipline 删除始终复用当前上传 Token，避免旧的复制请求头导致鉴权失败；Freeimage.host 等无公开删除接口的服务会安全保留远程文件并提示。
