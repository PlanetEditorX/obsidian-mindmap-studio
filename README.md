...
- “截图与识别”是位于“AI 助手”之后的独立设置分类。普通截图与截图并识别分别使用 `Ctrl/Cmd+Shift+S` 和 `Ctrl/Cmd+Shift+R`，可独立录制；节点右键和顶部工具栏均提供两种入口。Windows 截图源覆盖完整虚拟桌面，编辑器可在“全部屏幕”和各块显示器之间切换，按原始像素比例显示与导出，不再因 DPI 或窗口宽高比拉伸。标注支持矩形、圆角矩形、椭圆、填充、颜色与粗细样式，文字使用画布内输入框，固定截图使用真正置顶、可拖动和可缩放的独立窗口。普通截图保持选区常驻，双击选区后复制并插入；截图并识别在拖拽选区或调整边框后启动 3 秒自动完成计时，选区内鼠标活动或继续调整边框会重置计时。
