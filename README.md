...
- 导图仍先计算完整布局，保证连接线和节点坐标一致；节点 DOM 按“当前节点关系链 → 当前视口 → 相邻视口 → 其余区域”分帧挂载，优先保证正在编辑和正在查看的区域可用。
- 文章模式先显示已激活按钮和加载反馈；已有页面会保持可见，新页面在隐藏渲染区按当前节点、兄弟节点、父节点及其兄弟优先填充，完成后原位淡入替换。首次进入则显示按当前文章视图宽高动态铺满的静态骨架，避免只有少量占位行、白屏、整页闪烁或鼠标光标在文本与加载状态之间反复切换。
