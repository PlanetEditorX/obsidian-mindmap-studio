...
- 文章模式先显示已激活按钮和加载反馈；首批正文完成后立即显示新页面，剩余章节继续按帧填充。已有页面在首批内容就绪前保持可见，首次进入使用铺满当前视图的静态骨架，不再等待整篇文章全部渲染后才开放阅读。 渐进补齐期间一旦用户开始滚动、拖动滚动条或使用翻页键，插件立即把视口控制权交给用户，后续批次不会再恢复打开瞬间的旧位置。 首次渲染完成后，插件会在私有缓存目录保存节点级静态快照；文件、层级、编号和文章样式未变化时，后续打开会先直接恢复完整正文，再分帧绑定交互。节点指纹只覆盖当前章节自身和渲染上下文，不递归扫描后代；局部修改只重建真正变化或因层级、编号改变而失效的节点。
