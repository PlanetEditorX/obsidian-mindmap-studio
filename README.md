...
- 文章模式先显示已激活按钮和加载反馈；首批正文完成后立即显示新页面，剩余章节继续按帧填充。已有页面在首批内容就绪前保持可见，首次进入使用铺满当前视图的静态骨架，不再等待整篇文章全部渲染后才开放阅读。 渐进补齐期间一旦用户开始滚动、拖动滚动条或使用翻页键，插件立即把视口控制权交给用户，后续批次不会再恢复打开瞬间的旧位置。
