...
- 文章模式采用目标窗口渲染：首次只挂载当前节点前后各约 5 KB 的真实章节 DOM，接近窗口顶部或底部时再按约 5 KB 扩展；向上补载会补偿新增高度，避免把当前段落顶走。它不使用零高度占位、隐藏舞台或后台全页 hydration，因此大文件不会在点击后同步创建全部章节。目录点击会先确保目标章节进入窗口，再按精确节点 ID 恢复 `ReadingLocation`；同一文件的目录项直接切换正文并聚焦节点，不再重新打开文件，显式跳转也不会被旧阅读位置异步覆盖。单次已挂载窗口仍通过 `WeakMap` 复用内容块规范化结果；文章编号、折叠、缩略导航、图片、表格、代码和行内编辑保持原有能力。
