...
- 图床卡片提供 Zipline v4、Zipline v3、ImgBB、Freeimage.host 和自定义预设。Zipline 预设填写认证 Token 后可保存文件 ID并在最后引用删除时调用对应版本删除接口；ImgBB 保存上传响应中的唯一删除链接；Freeimage.host 的公开 API 只提供上传，插件不会假设存在匿名删除 API。
...
- 远程镜像与本地源并存；节点渲染和点击放大图片会先尝试优先级最高的可用图床地址，失败后再尝试下一个；仅在所有选定图床上传成功且确认没有其他引用时，才允许删除本地文件。图片块被删除后，只有开启“最后引用删除时同步清理图床”、确认全部 `.mindmap` 已无同一 SHA-256/远程 URL 引用，且预设或自定义配置提供了可验证删除接口时，才请求远程删除；Freeimage.host 等无公开删除接口的服务会安全保留远程文件并提示。
