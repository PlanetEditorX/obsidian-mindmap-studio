...
图片识图转文字后，所在节点默认左对齐。包含多个文字块或图片块的节点可直接双击目标文字块进行行内编辑；双击图片块会打开并高亮对应的完整编辑卡片。快速编辑与“编辑节点内容”现在共用加粗、斜体、下划线快捷键配置，选中文字后可在两处使用相同按键组合。图片放大预览采用较收敛的 `86vw × 82vh` 上限，避免覆盖几乎整个 Obsidian 窗口。
