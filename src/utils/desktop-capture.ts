...
<div data-style-group="arrow"><button class="line-option active" data-line-style="arrow">箭头</button><button class="line-option" data-line-style="line">直线</button><button class="line-option" data-line-style="tapered">渐粗</button><span class="sep"></span></div>
...
<input type="range" id="widthRange" min="1" max="20" step="1" value="4" class="width-range" aria-label="线宽 1 到 20"><span id="widthValue" class="width-value">4</span><span class="sep"></span><button data-fill="toggle">填充</button>
</div>`;
  return `<!doctype html>
...
(() => {
  ...
  const widthRange=document.getElementById('widthRange'); const widthValue=document.getElementById('widthValue'); function syncWidthUI(){widthRange.value=strokeWidth;widthValue.textContent=strokeWidth} widthRange.addEventListener('input',()=>{strokeWidth=Math.max(1,Math.min(20,Math.round(Number(widthRange.value)||4)));syncWidthUI();resetAutoConfirm()});
  ...
  function drawArrow(ctx,a,b){const angle=Math.atan2(b.y-a.y,b.x-a.x),dirx=Math.cos(angle),diry=Math.sin(angle),perpx=-diry,perpy=dirx;if(lineKind==='line')return drawLine(ctx,a,b);applyStyle(ctx);const head=Math.max(13,strokeWidth*3.2),bx=b.x-head*dirx,by=b.y-head*diry;if(lineKind==='tapered'){const tail=Math.max(1.2,strokeWidth*0.4),half=Math.max(2.5,strokeWidth*0.6);ctx.beginPath();ctx.moveTo(a.x-perpx*tail,a.y-perpy*tail);ctx.lineTo(bx-perpx*half,by-perpy*half);ctx.lineTo(b.x,b.y);ctx.lineTo(bx+perpx*half,by+perpy*half);ctx.lineTo(a.x+perpx*tail,a.y+perpy*tail);ctx.closePath();ctx.fill();return}ctx.lineCap='butt';ctx.lineJoin='miter';ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(bx,by);ctx.stroke();const wing=Math.max(2,head*0.42);ctx.beginPath();ctx.moveTo(b.x,b.y);ctx.lineTo(bx+perpx*wing,by+perpy*wing);ctx.lineTo(bx-perpx*wing,by-perpy*wing);ctx.closePath();ctx.fill()}
function drawLine(ctx,a,b){applyStyle(ctx);ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke()}
  function commitPreview(){actx.drawImage(preview,0,0,innerWidth,innerHeight);pctx.clearRect(0,0,innerWidth,innerHeight)}
  function mosaicAt(p){const size=Math.max(12,strokeWidth*3),small=Math.max(2,Math.floor(size/6)),sx=Math.max(imageArea.x,Math.min(imageArea.x+imageArea.w-size,rect.x+p.x-size/2)),sy=Math.max(imageArea.y,Math.min(imageArea.y+imageArea.h-size,rect.y+p.y-size/2)),tmp=document.createElement('canvas');tmp.width=small;tmp.height=small;const t=tmp.getContext('2d');t.drawImage(base,sx*dpr,sy*dpr,size*dpr,size*dpr,0,0,small,small);actx.save();actx.imageSmoothingEnabled=false;actx.drawImage(tmp,0,0,small,small,sx,sy,size,size);actx.restore()}
