...
(() => {
  ...
  let tool=''; let drawing=false; let start=null; let number=1; let drag=null; let selectionDraw=null; let autoConfirmTimer=null; let autoConfirmTicker=null; let autoConfirmDeadline=0; let autoConfirmArmed=false; let activeTextPoint=null;
  ...
  tip.textContent=recognizeMode?'3 秒无操作后自动识别；移动鼠标或调整边框会重新计时，Esc 取消':'拖动或调整蓝色边框；双击选区复制并插入节点，Esc 取消';
  ...
  function renderCountdown(){if(!recognizeMode||!autoConfirmArmed){countdown.classList.remove('show');countdown.textContent='';return}const remaining=Math.max(0,autoConfirmDeadline-Date.now());countdown.classList.add('show');countdown.textContent=(remaining/1000).toFixed(1)+' 秒后自动识别'} function clearAutoConfirm(){if(autoConfirmTimer!==null){clearTimeout(autoConfirmTimer);autoConfirmTimer=null}if(autoConfirmTicker!==null){clearInterval(autoConfirmTicker);autoConfirmTicker=null}autoConfirmDeadline=0;countdown.classList.remove('show');countdown.textContent=''} function scheduleAutoConfirm(){if(!recognizeMode||!autoConfirmArmed)return;clearAutoConfirm();autoConfirmDeadline=Date.now()+autoConfirmDelayMs;renderCountdown();autoConfirmTicker=setInterval(renderCountdown,100);autoConfirmTimer=setTimeout(()=>{if(drag||selectionDraw||drawing||textEditor.style.display==='block'){scheduleAutoConfirm();return}action('copy')},autoConfirmDelayMs)} function armAutoConfirm(){if(!recognizeMode)return;autoConfirmArmed=true;scheduleAutoConfirm()} function resetAutoConfirm(){if(recognizeMode&&autoConfirmArmed)scheduleAutoConfirm()}
  ...
  toolbar.querySelectorAll('[data-action]').forEach(btn=>btn.addEventListener('click',()=>action(btn.dataset.action)));document.addEventListener('pointermove',ev=>{if(recognizeMode&&autoConfirmArmed&&insideRect(ev.clientX,ev.clientY)&&!selectionDraw&&!drag)resetAutoConfirm()});toolbar.addEventListener('pointermove',resetAutoConfirm);toolbar.addEventListener('pointerdown',resetAutoConfirm);stylebar.addEventListener('pointermove',resetAutoConfirm);stylebar.addEventListener('pointerdown',resetAutoConfirm);
...
async function editCapturedDisplay(
  ...
  return await new Promise<DesktopCaptureResult>((resolve, reject) => {
    ...
    const cleanup = (): void => {
      ...
      if (onHostKeydown) window.removeEventListener("keydown", onHostKeydown, true);
