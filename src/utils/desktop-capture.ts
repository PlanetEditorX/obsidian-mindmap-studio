...
(() => {
  ...
  function drawArrow(ctx,a,b){const angle=Math.atan2(b.y-a.y,b.x-a.x),dirx=Math.cos(angle),diry=Math.sin(angle),perpx=-diry,perpy=dirx;if(lineKind==='line')return drawLine(ctx,a,b);applyStyle(ctx);const head=Math.max(13,strokeWidth*3.2),bx=b.x-head*dirx,by=b.y-head*diry;ctx.lineCap='butt';ctx.lineJoin='miter';ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(bx,by);ctx.stroke();const wing=Math.max(2,head*0.42);ctx.beginPath();ctx.moveTo(b.x,b.y);ctx.lineTo(bx+perpx*wing,by+perpy*wing);ctx.lineTo(bx-perpx*wing,by-perpy*wing);ctx.closePath();ctx.fill()}
