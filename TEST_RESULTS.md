# Markdown 导入图片引用与自动上传验证

## 本次修改

- 修复图片文件复制成功后节点 `content[].source`、`localSource` 和 `node.image` 仍保留原 `assets/...` 路径的问题。
- 新复制资源优先按完整仓库路径直接解析，元数据缓存未及时索引也能立即显示。
- 导入为子节点或替换整图后，在最终节点 ID 已确定的状态下安排本地图片自动上传。
- Markdown 转脑图保存新文件后恢复待上传任务，沿用既有延迟、图床选择和本地删除安全策略。

## 自动验证

- 完整单元测试：`195 / 195` 通过。
- 综合回归测试：通过，输出 `All MindMap Studio tests passed.`。
- 文档检查：`49` 个 TypeScript 模块中的 `911` 个具名声明均有 JSDoc。
- 仓库检查：通过。
- TypeScript 类型检查和生产构建：通过。
- 桌面图片路径读取、权威内容块改写、兼容镜像、直接路径解析和自动上传排程专项测试均通过。

## 待手工验证

- 在真实 Obsidian 中导入包含 `![[assets/公文/图片.png|L]]` 的 Markdown，确认节点图片路径改为当前导图资源文件，并在启用自动上传后按设定延迟切换到图床地址。
