...
- 导图仍一次计算完整布局和连接线，但节点 DOM 改为按当前节点关系链、当前视口、相邻视口和其他区域的顺序分帧挂载。
- 导图、文章和大纲中的纯文字提交只刷新当前节点或保留现有文章 DOM，不再触发整页重建；历史记录、自动保存和权威内容块写回保持不变。
- 文章模式先让按钮激活和加载态完成一次绘制，再创建轻量章节占位并按当前节点、兄弟、父节点及父级兄弟优先填充。
- 文章渐进加载完成后恢复语义阅读位置，并安装章节折叠、回到顶部、缩略导航和点击移动交互。
- 加载与占位动画遵循系统“减少动态效果”设置。

## 自动验证

- 完整单元测试：`204 / 204` 通过。
- 综合回归测试：通过，输出 `All MindMap Studio tests passed.`。
- 文档检查：`50` 个 TypeScript 模块中的 `929` 个具名声明均有 JSDoc。
- 仓库检查：通过。
- TypeScript 类型检查和生产构建：通过，`main.js` 已重新生成。
- `node --check main.js`：通过。
- 新增层级聚焦、当前/相邻视口、文章填充顺序、局部文字提交和文章入口延迟绘制专项契约测试。

## 待手工验证

- 在真实 Obsidian 桌面端打开超大导图，确认当前节点及当前视口先出现，剩余节点分帧补齐期间仍可平移和缩放。
- 在导图、文章和大纲中编辑文字并按 Enter 或失焦保存，确认不再出现数秒整页卡顿，撤销和自动保存仍正常。
- 从其他模式点击进入超大文章，确认按钮立即松开并显示加载反馈，正文渐进出现后仍定位在原节点而不是跳回顶部。
- 验证大型文章中的图片、代码、表格、章节折叠、缩略导航和点击移动在渐进加载完成后正常工作。
