...
- 移除垂直时间线主题（枚举/渲染/CSS/下拉）；「卡片」更名「经典卡片」，九种主题名称统一四字。
- `npm run verify`（本机完整执行）：`test:unit` **405 / 405 通过**；`test:regression` 全部通过；`test:docs` 通过；`test:repo` 通过；production esbuild 通过。

## 1.49.4 暗色玻璃升级为透明玻璃
## 1.49.4 暗色玻璃升级为透明玻璃

- `toc-glass` 移除固定深色底，改为半透明磨砂面板跟随主题明暗与强调色；标签更名「透明玻璃」。
- `npm run verify`（本机完整执行）：`test:unit` **405 / 405 通过**；`test:regression` 全部通过；`test:docs` 通过；`test:repo` 通过；production esbuild 通过。

### 仍需手工验证

- 明色与暗色 Obsidian 主题下透明玻璃的通透感与可读性。

## 1.49.3 目录主题二次调整
## 1.49.3 目录主题二次调整

- 移除书脊索引/编辑部极简；暗色玻璃跟随主题强调色；杂志网格去除子条目“—”前缀；新增极光列表/墨韵书卷/落日暖橙，目录主题合计 10 种。
- `npm run verify`（本机完整执行）：`test:unit` **405 / 405 通过**；`test:regression` 全部通过；`test:docs` 通过；`test:repo` 通过；production esbuild 通过。

### 仍需手工验证

- 真实桌面端逐一预览 10 种目录主题；暗色玻璃在明/暗 Obsidian 主题与不同强调色下的观感；墨韵书卷作为书籍预设默认目录的整体搭配。

## 1.49.2 目录主题重整
## 1.49.2 目录主题重整

- 目录样式 9 种：保留卡片/简洁列表/素雅面板/极简书页，新增杂志网格/垂直时间线/编辑部极简/暗色玻璃/书脊索引（设计稿移植）；移除引导线/现代报告/杂志索引旧实现/层级树线，历史值自动回退。
- `renderDirectory` 新增章节分组与书脊索引侧轨过滤布局；书籍预设目录改为编辑部极简。
- `npm run verify`（本机完整执行）：`test:unit` **405 / 405 通过**；`test:regression` 全部通过；`test:docs` 覆盖 62 个源码模块；`test:repo` 通过；production esbuild 通过。

### 仍需手工验证

- 真实桌面端逐一切换 9 种目录主题检查观感；书脊索引侧轨按章过滤；杂志/玻璃分组卡组头跳转；书籍预设下的编辑部极简目录。
...
- 修复：`.mms-article-view` 显式 `overflow-anchor: none`；懒加载补偿成为唯一补偿源。契约测试锁定该 CSS 规则。
- `npm run verify`（本机完整执行）：`test:unit` **404 / 404 通过**；`test:regression` 全部通过；`test:docs` 覆盖 62 个源码模块；`test:repo` 通过；production esbuild 通过。

### 仍需手工验证

- 真实桌面端：导图聚焦节点 → 切文章 → 滚轮滚动，视口应稳定停在聚焦节点附近，上方内容补载不产生任何跳变；右侧导航条随预热逐步补全。

## 1.48.10 修复契约拼接的跨文件误报
## 1.48.10 修复契约拼接的跨文件误报

- 根因：编辑器源码契约改为多文件拼接后，`doesNotMatch(/A[\s\S]*B/)` 类契约的作用域从单文件扩大到拼接全文，节点渲染模块中合法的 `editSelected(blockId)` 被误判为表格双击回归（CI 捕获，本地当时该路径未触发）。
- 修复：表格双击契约限定在处理器体内检查；24 处引用已迁移成员的断言统一接受 `this.` / `ctx.` 前缀；补回两处缺失 JSDoc。
- `npm run verify`（本机完整执行）：`test:unit` **403 / 403 通过**；`test:regression` 全部通过；`test:docs` 覆盖 62 个源码模块、1264 个具名声明；`test:repo` 通过；production esbuild 通过。

## 1.48.9 编辑器拆分第三批：导图节点渲染模块
## 1.48.9 编辑器拆分第三批：导图节点渲染模块

- `renderMindMapNode`（398 行）迁移为 `mind-map-node-renderer.ts` 纯渲染函数；可变拖拽状态经 get/set 闭包接回编辑器，实时读取字段用 getter，DOM 输出与拆分前一致。
- `npm run verify`（本机完整执行）：`test:unit` **403 / 403 通过**；`test:regression` 全部通过；`test:docs` 覆盖 61 个源码模块、1261+ 个具名声明；`test:repo` 通过；production esbuild 通过。
- 拆分剩余批次：题目系统流程、行内编辑深化。

## 1.48.8 编辑器拆分第二批：视口控制器
## 1.48.8 编辑器拆分第二批：视口控制器

- 视口状态与变换机制迁移到 `ViewportController`（约 190 行）；编辑器经存取器转发保持全部既有引用与调用点零改动，监听注册原地保留（与选区逻辑交织，行为逐字节不变）。
- `npm run verify`（本机完整执行）：`test:unit` **403 / 403 通过**；`test:regression` 全部通过；`test:docs` 覆盖 61 个源码模块、1261 个具名声明；`test:repo` 通过；production esbuild 通过。
...
- `src/editor/editor.ts`：9,094 → 7,842 行；`NodeEditModal`（618 行）与 `AppearanceModal`（675 行）拆分为独立模块，沿用构造参数 + 回调注入的组合模块模式，不共享实例状态。
- 契约测试与源码解耦：编辑器核心契约统一读取三文件拼接内容（含综合回归），后续拆分批次无需逐个改指向。
- `npm run verify`（本机完整执行）：`test:unit` **403 / 403 通过**；`test:regression` 全部通过（bundle 契约继续覆盖拆分后的安装 `main.js`）；`test:docs` 覆盖 **60 个源码模块、1249 个具名声明**；`test:repo` 通过；production esbuild 通过。
- 拆分后续批次（建议单独排期）：视口手势与缩放、行内编辑与富文本协调、题目系统流程——三者与实例状态耦合更深，需按“上下文对象 + 独立类”模式逐批迁移并保持契约绿。

## 1.48.6 AI 客户端完成结果汇总去重
## 1.48.6 AI 客户端完成结果汇总去重

- 审查清单第 4 项：`src/ai/client.ts` 抽取 `buildCompletionResult()`，四个公开请求函数复用同一文本/模型/usage 汇总逻辑，删除四处重复实现；空结果错误消息按调用方保留。
- `npm run verify`（本机完整执行）：`test:unit` **403 / 403 通过**（新增 1 项去重契约）；`test:regression` 全部通过；`test:docs` 通过；`test:repo` 通过；production esbuild 通过。
...
- 审查清单第 2 项：`documentSnapshotJson` 缓存新增低频抽样自愈校验（10 秒间隔）。缓存命中读取时与当前文档新鲜序列化比对，不一致立即失效缓存并回退完整序列化，同时发出 `document-snapshot-cache-mismatch` 调试事件与控制台错误；正确代码下该校验永不触发，运行时行为不变。
- 审查清单第 3 项：节点树索引新增 `nodeTreeIndexStale` 过期标记。`captureHistorySnapshot()`（全部可撤销写操作前置入口）、`mutate()` 执行动作之后与拖拽批量移动流程标记过期；`currentNodeTreeIndex()` 在过期/缺失/根替换三条件下重建；`render()` 重建后复位。结构性修改与索引读取之间的时序安全不再依赖“修改后立即 render”的调用约定。
- `npm run verify`（本机完整执行）：`test:unit` **402 / 402 通过**（含 5 项新增源码契约）；`test:regression` 全部通过；`test:docs` 覆盖 58 个源码模块、1249 个具名声明；`test:repo` 通过；production esbuild 通过。

### 兼容性边界

- 抽样校验每 10 秒最多一次全量序列化，仅在缓存命中读取时执行；大型导图的额外成本可忽略（10k 节点约数毫秒、每 10 秒至多一次）。
- 索引过期标记不主动重建，只在下一次读取时多一次 DFS——该读取在不标记时也会由 `render()` 全量重建，无净增成本。
...
- 右键菜单按状态区分默认来源：已固定的来源显示“取消默认显示来源”，取消后回退全局优先级并按需移除 `sourcePriority`；未固定的显示“设为默认显示来源”。
- 更新动作按来源类型区分：图床/手动 URL 为“更新上传（选择本地图片并上传图床）”（图床选择 → 文件选择器 → 上传合并镜像）；本地副本为“更新替换（选择本地图片）”（文件选择器 → `onSavePastedImage` 存库 → 更新 `localSource`，不触碰图床镜像）。
- 来源栏“＋”按钮默认隐藏手动 URL 输入面板，点击展开并聚焦，提交后自动收起；预览图片支持指针拖拽平移，切换来源与双击复位时平移归零。
- `npm run verify`（本机完整执行）：`test:unit` **401 / 401 通过**（`tests/image-source-candidates.test.mjs` 14 项，含取消默认纯函数与更新后的接线契约）；`test:regression` 全部通过；`test:docs` 覆盖 58 个源码模块、1247 个具名声明；`test:repo` 通过；production esbuild 通过。

### 仍需手工验证

- 真实桌面端：固定/取消默认来源的菜单状态切换与显示顺序回退；本地副本“更新替换”后本地来源正确显示；“＋”展开添加 URL；放大后拖拽平移的手感。

## 1.48.2 图片预览更新上传接入文件选择器
## 1.48.2 图片预览更新上传接入文件选择器

### 实现与行为验证

- 修复“更新上传”未打开文件管理器的问题：右键“更新上传（选择本地图片并上传图床）”现在依次打开图床选择弹窗与系统文件选择器，上传成功后按图床合并镜像、更新默认来源并实时刷新预览来源栏；取消任一步不产生撤销记录。
- 新增 `.gitattributes`（`* text=auto eol=lf`）并统一行尾：修复 Windows `core.autocrlf=true` 下 rebase 重新检出把源码转成 CRLF，导致按 `
` 匹配源码的 4 项契约测试（incremental-render ×2、reading-editor-contract ×2）误报的问题；仓库内与工作区今后均保持 LF。
- `npm run verify`（本机完整执行）：`test:unit` **400 / 400 通过**（含更新后的来源管理契约）；`test:regression` 全部通过；`test:docs` 覆盖 58 个源码模块、1246 个具名声明；`test:repo` 通过；production esbuild 通过。

### 仍需手工验证

- 真实桌面端：右键来源“更新上传”→ 选择图床 → 文件管理器选择本地图片 → 上传成功后预览与导图默认来源切换；取消选图不产生撤销条目。
