# 图片自动上传、页面批量上传与节点布局稳定性验证

## 本次修改

- 自动上传任务改为持有当前导图 `TFile` 对象；保存前若“中心节点标题同步文件名”导致导图重命名，任务会按更新后的路径继续读取和写回。
- 导图画布空白处右键新增“上传当前页面所有图片”，一次选择图床，只补传当前页面中尚未覆盖所选图床的图片，并统一写回远程镜像及主显示地址。
- 导图节点 `ResizeObserver` 增加实际外框尺寸缓存；点击、选中样式等未改变尺寸的通知不再触发碰撞布局，避免视口锚点被反复累计而持续向下移动。
- 图片、表格或代码等确有尺寸变化时仍会执行一次测量布局并保持当前节点屏幕位置。
...
- 导图重命名后的自动上传、当前页面批量上传、缺失图床过滤、权威内容块写回和无尺寸变化不重排专项测试均通过。

## 待手工验证

- 开启自动上传、默认图床和“中心节点标题同步文件名”，导入包含本地图片的 Markdown；确认保存导致导图重命名后，图片仍按设定延迟自动上传并切换为图床地址。
- 在画布空白处右键执行“上传当前页面所有图片”，确认只选择一次图床，已覆盖所选图床的图片被跳过，缺失镜像得到补传。
- 连续点击不同节点并等待，确认整张导图不再持续下移；再加载一张尺寸变化的图片，确认布局只进行必要的一次调整。
