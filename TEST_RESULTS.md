...
- 文章重建不再先清空当前可见页面；旧页保持显示并暂时禁止交互，新页在隐藏渲染区完成分帧挂载后再原位替换。
- 首次进入文章模式且没有旧页面时显示静态文章骨架和加载状态，避免白屏后整页突然出现。
- 新页面交换时使用短暂淡入，加载提示淡出；系统“减少动态效果”启用时关闭旋转和过渡动画。
- 移除文章加载期间对整个滚动容器的 `cursor: progress`，避免鼠标停留在正文或空白区域时光标在文本、默认和加载样式之间反复闪烁。
- 取消、覆盖或切换模式时统一清理隐藏渲染区、加载层、骨架、保留页状态和延迟过渡计时器。

## 自动验证

- 完整单元测试：`207 / 207` 通过。
- 新增文章双缓冲渲染、旧页保留、首次骨架、无全局 progress 光标、淡入淡出与取消清理专项契约测试。
- TypeScript 类型检查、综合回归、文档检查、仓库检查、生产构建和 `main.js` 语法检查在本轮完整验证中执行。

## 待手工验证

- 在真实 Obsidian 桌面端打开大型文章，将鼠标停在正文、标题、空白区域和右侧导航附近，确认光标不持续闪烁。
- 在文章模式中输入、移动、删除或折叠节点，确认重建期间旧内容保持可见，加载提示出现，新页淡入时没有白屏或整页闪一下。
- 从导图首次切换到尚未渲染过的超大文章，确认立即显示骨架和加载状态，完成后保持原阅读位置。
- 启用系统“减少动态效果”后确认功能正常且不播放旋转、淡入或淡出动画。

# 大型文章编辑后滚动位置稳定性修复验证

## 本次修改

- 文章渐进重建前记录旧 `scrollTop`、`scrollLeft` 和 `scrollHeight`，加载占位与文章页面在重建期间维持旧滚动范围，避免浏览器将容器立即收缩到顶部。
- 文章渲染器每完成一批章节即回调编辑器；目标语义节点已挂载时恢复节点内部比例和视口比例，尚未挂载时继续使用旧像素滚动位置兜底。
- 渐进加载完成后移除临时高度并再次恢复语义位置，兼容图片、字体和代码块等后续布局变化。
- 异步文章族上下文刷新仅在目录层级、目录项或分页导航变化时重建当前文章；普通文字保存只更新跨文件阅读快照，不再在 320ms 后二次重绘页面。

## 自动验证

- 完整单元测试：`206 / 206` 通过。
- 综合回归测试：通过，输出 `All MindMap Studio tests passed.`。
- 文档检查：`50` 个 TypeScript 模块中的 `932` 个具名声明均有 JSDoc。
- 仓库检查：通过。
- TypeScript 类型检查和生产构建：通过，`main.js` 已重新生成。
- `node --check main.js`：通过。
- 新增文章旧滚动高度保留、逐批语义锚点校正、目标未挂载像素兜底和上下文刷新跳过冗余重建专项契约测试。

## 待手工验证

- 在真实 Obsidian 桌面端打开超大文章，滚动到页面中部后编辑标题或正文并按 Enter/失焦保存，确认视图不跳到顶部且光标提交正常。
- 在页面中部新增、删除、移动或折叠节点，确认渐进重建期间视口保持在原语义节点，完成后没有二次漂移。
- 验证当前节点之前存在大量图片、代码和表格时，异步布局完成后阅读锚点仍稳定。
- 验证文章目录层级、跨文件目录或分页导航确实变化时仍会正确重建并保持当前位置。
