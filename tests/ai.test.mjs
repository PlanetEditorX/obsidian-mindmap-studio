...
test("AI integration exposes toolbar, shortcut, page scope and node scope contracts", async () => {
  ...
  assert.match(settingsSource, /"AI 助手", "截图与识别", "管理配置"/);
  assert.match(settingsSource, /containerEl\.createEl\("h3", \{ text: "截图与识别" \}\)/);
  ...
  assert.match(settingsSource, /screenshotRecognizeShortcut: "Ctrl\+Shift\+R"/);
  ...
  assert.match(mainSource, /id: "capture-and-recognize-mind-map-screenshot"/);
  assert.match(mainSource, /modifiers: \["Mod", "Shift"\], key: "S"/);
  assert.match(mainSource, /modifiers: \["Mod", "Shift"\], key: "R"/);
  ...
  assert.match(mainSource, /!stored\.includes\("screenshot-recognize"\)/);
  ...
  assert.match(editorSource, /this\.shortcutMatches\(event, this\.options\.screenshotRecognizeShortcut\)/);
  assert.match(editorSource, /this\.options\.screenshotShortcut \|\| "Ctrl\+Shift\+S"/);
  assert.match(editorSource, /this\.options\.screenshotRecognizeShortcut \|\| "Ctrl\+Shift\+R"/);
  assert.match(editorSource, /setTitle\("插入截图"\)/);
  assert.match(editorSource, /setTitle\("插入截图并识别"\)/);
  ...
  assert.match(editorSource, /captureScreenshot\(recognizeAfter = false/);
  ...
  assert.match(viewSource, /screenshotRecognizeShortcut: this\.plugin\.settings\.screenshotRecognizeShortcut/);
