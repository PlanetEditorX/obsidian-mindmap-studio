...
`);
  ...
  const xmindFixture = model.createDefaultDocument("XMind style");
  const cardLayout = layout.computeLayout(xmindFixture.root, "right", 14, "card");
  const xmindLayout = layout.computeLayout(xmindFixture.root, "right", 14, "xmind");
  assert.ok(xmindLayout.nodes[1].width < cardLayout.nodes[1].width, "XMind style should fit node width to its text");
  assert.match(layout.roundedElbowEdgePath(xmindLayout.nodes[0], xmindLayout.nodes[1]), /\bQ\b/, "XMind style should use rounded elbow connectors");
  xmindFixture.appearance = { nodeVisualStyle: "compact" };
  assert.equal(model.normalizeDocument(xmindFixture).appearance?.nodeVisualStyle, "xmind", "legacy compact style should migrate to XMind style");
