...
| 梨 | 2 | 进行中 |`;
  ...
  assert.doesNotMatch(editorSource, /element\.setAttr\("aria-label", placeholder\)/, "inline article text should not expose paragraph or heading hover labels");
  ...
  assert.match(editorSource, /34 \+ emphasis \* 14/, "the nearest minimap marker must expand while neighbours taper");
  assert.match(editorSource, /depth === highestDepth \? 7 : 3/, "the highest and next-highest levels must use different marker thicknesses");
  ...
  assert.match(cssSource, /\.mms-article-minimap[\s\S]*width:\s*54px[\s\S]*padding:\s*0/, "the reading minimap must remain a narrow, background-free marker strip");
