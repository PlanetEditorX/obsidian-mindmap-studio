...
| 梨 | 2 | 进行中 |`;
  ...
  assert.match(cssSource, /\.mmc-fold-button\s*\{[\s\S]*top:\s*50%[\s\S]*transform:\s*translateY\(-50%\)/, "fold controls must sit at the node midline instead of overlapping the resize handle");
  ...
  assert.match(editorSource, /this\.buildUi\(\);[\s\S]*if \(this\.resizeModifier === "ctrl"\) this\.rootEl\.addClass\("mmc-ctrl-resize"\)/, "Ctrl resize styling must be applied after the editor root exists");
  assert.match(editorSource, /private resizeModifier: ResizeModifier = "ctrl"/, "Ctrl/Cmd resize must be the editor default");
  assert.match(settingsSource, /export type ResizeModifier = "none" \| "ctrl"/);
  assert.match(settingsSource, /resizeModifier: "ctrl"/, "new installations must default to Ctrl/Cmd resize");
  assert.match(settingsSource, /\.setName\("拖动调整节点大小"\)[\s\S]*\.addOption\("ctrl", "按住 Ctrl \/ Cmd"\)[\s\S]*\.addOption\("none", "直接拖动"\)/, "settings must expose both resize activation modes");
