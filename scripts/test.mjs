...
| 梨 | 2 | 进行中 |`;
  ...
  const searchShortcutIndex = handleKeydownSource.indexOf("if (mod && findKey && !event.altKey)");
  ...
  assert.match(handleKeydownSource, /if \(mod && findKey && !event\.altKey\) \{[\s\S]*?event\.preventDefault\(\);[\s\S]*?event\.stopPropagation\(\);[\s\S]*?this\.callbacks\.onGlobalSearch\(\)[\s\S]*?this\.openSearch\(\)/);
