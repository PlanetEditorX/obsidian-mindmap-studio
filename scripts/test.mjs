...
| 梨 | 2 | 进行中 |`;
  ...
  assert.match(editorSource, /item\.style\.setProperty\("--mms-article-depth", String\(entry\.depth\)\)/, "continuous-reading TOC must visually distinguish chapter and section depth");
  ...
  assert.match(editorSource, /scrollWheelAction === "pan"[\s\S]*this\.panX -= event\.deltaX/, "scroll-wheel action should support canvas panning");
  ...
  assert.match(settingsSource, /scrollWheelAction/);
  assert.match(settingsSource, /滚轮手势/);
  ...
  assert.match(mainSource, /scrollWheelAction: raw\.scrollWheelAction === "pan" \? "pan" : "zoom"/, "stored scroll-wheel gesture settings must be normalized");
