...
| 梨 | 2 | 进行中 |`;
  ...
  const searchShortcutIndex = handleKeydownSource.indexOf("if (mod && event.altKey && findKey && !event.shiftKey)");
  ...
  assert.match(handleKeydownSource, /if \(mod && event\.altKey && findKey && !event\.shiftKey\) \{[\s\S]*?event\.preventDefault\(\);[\s\S]*?event\.stopPropagation\(\);[\s\S]*?this\.openSearch\(\)/);
  ...
  assert.match(editorSource, /this\.currentMode === "reading" \|\| this\.currentMode === "question-bank"[\s\S]*this\.document\.view\?\.readOnly === true[\s\S]*this\.currentMode === "article" && this\.options\.articleEntryLockMode === "locked"/, "reading and question-bank must initialize locked; article follows its configured entry policy");
  assert.match(editorSource, /if \(mode === "article" && mode !== previousMode\) \{[\s\S]*articleEntryLockMode === "locked" \? true : this\.readOnly[\s\S]*else if \(\(mode === "reading" \|\| mode === "question-bank"\) && mode !== previousMode\) \{[\s\S]*this\.readOnly = true/, "article entry must honor its policy while reading and question-bank always lock");
