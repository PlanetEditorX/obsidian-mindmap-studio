...
import { loadEditorSources } from "../tests/helpers/editor-sources.mjs";
...
`);
  ...
  assert.equal(reopened.articleStyle?.tocStyle, "ink", "the book preset must now carry the ink toc style");
...
| 梨 | 2 | 进行中 |`;
  ...
  assert.match(mainSource, /private globalSearchModal: GlobalMindMapSearchModal \| null = null/, "global search should retain one mounted modal instance");
  assert.match(mainSource, /private globalSearchLaunchPending = false/, "global search should deduplicate launches while the index-ready await is pending");
  assert.match(mainSource, /openGlobalSearch\(\): void \{[\s\S]*globalSearchModal\?\.isMounted\(\)[\s\S]*open-deduplicated[\s\S]*globalSearchLaunchPending = true/, "global shortcut handling should not create duplicate modal layers");
  const mapFamilySearchSource = mainSource.slice(mainSource.indexOf("async openMapFamilySearch("), mainSource.indexOf("async rebuildGlobalSearchIndex("));
  assert.doesNotMatch(mapFamilySearchSource, /globalSearchModal|globalSearchLaunchPending|open-deduplicated/, "map-family search must remain independent from the global-search singleton guard");
  assert.match(globalSearchSource, /role: "button", tabindex: "0"/, "result rows must not nest a replace button inside another button");
  assert.doesNotMatch(globalSearchSource, /hostClosePromise|resolveHostClose|waitForHostClose/, "search result navigation must not block on an onClose promise");
  assert.match(globalSearchSource, /this\.shouldRestoreSelection = false;[\s\S]*this\.close\(\);/, "search navigation should request exactly one native Modal.close after disabling selection restore");
  assert.equal((globalSearchSource.match(/this\.close\(\);/g) ?? []).length, 1, "search result path must not issue duplicate Modal.close calls");
  assert.doesNotMatch(globalSearchSource, /backdrop\.click\(\)|querySelector<HTMLElement>\("\.modal-bg"\)|new PointerEvent|dispatchEvent/, "search result path must not synthesize backdrop events");
  assert.match(globalSearchSource, /waitForResultNavigationTurn\(\)[\s\S]*requestAnimationFrame\(\(\) => ownerWindow\.requestAnimationFrame/, "search navigation should yield two paint frames after native close without waiting on onClose");
  ...
  const editorSource = await loadEditorSources();
  ...
  const mindMapRendererSource = await readFile("src/editor/mind-map-node-renderer.ts", "utf8");
  ...
  assert.match(richTextDomSource, /renderInlineMarkdown\(container: HTMLElement, markdown: string\): void \{[\s\S]*renderRichTextRuns\(container, parsed\.richText, parsed\.text, true\)/, "table cells must render LaTeX formulas, not just inline Markdown");
  ...
  assert.match(cssSource, /\.mmc-shift-resize \.mmc-node > \.mmc-node-resize-handle[\s\S]*display:\s*none !important/, "Shift-resize must hide every handle until its node is hovered");
  assert.match(cssSource, /\.mmc-shift-resize\.is-shift-held \.mmc-node:hover > \.mmc-node-resize-handle[\s\S]*display:\s*block !important/, "Shift-resize must reveal only the hovered node's handle");
  assert.match(cssSource, /\.mmc-shift-resize\.is-shift-held \.mmc-node:hover > \.mmc-fold-button[\s\S]*display:\s*none/, "the child counter must hide while the hovered node shows its resize handle");
  ...
  assert.match(editorSource, /this\.rootEl\.addClass\("mmc-shift-resize"\)/, "Shift-resize styling must be applied after the editor root exists");
  ...
  // Modifier swap: multi-select is Ctrl/Cmd (individual click toggles and canvas
  // drag marquee), while node resize uses Shift instead of the old ^Shift->Ctrl split.
  assert.match(mindMapRendererSource, /if \(event\.ctrlKey \|\| event\.metaKey\) \{\s*\n\s*ctx\.toggleNodeSelection\(node\.id\)/, "Ctrl/Cmd click must toggle a node's selection");
  assert.match(editorSource, /if \(event\.button === 0 && \(event\.ctrlKey \|\| event\.metaKey\)\) \{/, "Ctrl/Cmd drag must start the marquee multi-select");
  assert.match(mindMapRendererSource, /if \(!event\.shiftKey\) return;/, "node resize must require Shift instead of Ctrl/Cmd");
  assert.match(editorSource, /toggleClass\("is-shift-held", trackEvent\.shiftKey\)/, "resize affordance must follow the live Shift state");
