...
`);
  ...
  const compactFixture = model.createDefaultDocument("Compact");
  const cardLayout = layout.computeLayout(compactFixture.root, "right", 14, "card");
  const compactLayout = layout.computeLayout(compactFixture.root, "right", 14, "compact");
  assert.ok(compactLayout.nodes[1].width < cardLayout.nodes[1].width, "compact style should reduce automatic node width");
