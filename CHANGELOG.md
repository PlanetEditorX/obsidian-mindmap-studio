...
- 文章编号新增“自动 / 关闭 / 手动层级”三种节点级配置，手动层级支持 1–8 级。
- 手动编号可将末端节点作为标题，支持“一、/（一）”和“1./（1）”等成语、提纲与讲义结构。
- 根节点可作为整张导图的文章层级基准；手动层级会在后代和跨子导图中继续传递。
- 不同有效文章层级改为独立计数，混合编号格式不会互相挤占序号。
- 文章正文、递归目录、通读模式和 HTML/Word/PDF 导出统一使用同一套层级解析规则。
