...
- 大纲模式改为会话级状态：当前会话内仍同步所有已打开导图，但不会成为插件重启后的默认启动模式；旧的 `outline` 设置自动回到导图或其他可见的持久化模式。
- 新增导图、大纲、文章和通读四模式统一阅读位置。点击节点或滚动到章节后切换模式，会聚焦同一物理文件和节点。
- 统一阅读位置按文章族持久化，关闭视图、重启和重新打开后恢复；通读中的子导图位置切换到其他模式时自动打开对应子导图。
- 目标节点删除时按保存的祖先链逐级回退；子导图缺失时回退到父导图挂载节点，目录页与子章节提供对应语义锚点，文件改名时同步迁移进度路径。
- 修复异步文章上下文刷新和多视图全局切换可能覆盖最新进度的问题：跨文章族切换先写回旧族状态，未发起切换的视图丢弃延迟写入。
...
- 抽取跨平台文件名、扩展名、时间戳和图片 MIME 纯工具，统一新建导图、资源保存与上传文件命名规则。
...
- 修复综合回归仍要求在 `editor.ts` 内联出现 `nodeRatio` 截断表达式的问题；比例规范化已迁移到 `reading-location.ts` 的共享 `clampRatio()`，契约检查现验证共享工具调用，边界行为继续由单元测试覆盖。
...
- 快速行内编辑粘贴已识别的节点载荷时仅插入节点文字，不再把内部 JSON 原始信息写进正文；缩略导航图提示仅显示对应章节名称。
...
- 阅读缩略导航图固定到右侧垂直正中心，并仅显示当前页面最高层级的章节/小节；滚动时当前类别对应的黑色短条随之移动。
- 阅读缩略导航图刻度条改为统一长度，并同时显示最高与次高结构层级：最高层使用粗条、次高层使用细条。
- 阅读缩略导航图移除横向底条；鼠标悬停时按距离放大最近刻度、渐变缩短相邻刻度，移开后恢复默认。
- 放大阅读缩略导航图及刻度尺寸，并限制导航区只允许纵向滚动，彻底移除底部横向滚动条。
- 移除阅读缩略导航图刻度的浏览器原生白色提示，仅保留 Obsidian 的黑色章节提示。
